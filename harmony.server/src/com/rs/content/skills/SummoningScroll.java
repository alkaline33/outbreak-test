	...
	public static void createScroll(Player player, int itemId, int amount) {
		...
		if (!player.getInventory().containsItem(scroll.pouchId, 1)) {
			player.getPackets().sendGameMessage("You do not have enough "
					+ ItemDefinitions.getItemDefinitions(scroll.pouchId).getName()
							.toLowerCase()
					+ "es to create "
					+ amount
					+ " "
					+ ItemDefinitions.getItemDefinitions(scroll.scrollId).getName()
							.toLowerCase() + "s.");
			return;
		}
		if (player.getSkills().getLevel(Skills.SUMMONING) < scroll.reqLevel) {
			player.getPackets().sendGameMessage("You need a summoning level of "
					+ scroll.reqLevel
					+ " to create "
					+ amount
					+ " "
		...
		player.getPackets().sendGameMessage("You have created some "
				+ ItemDefinitions.getItemDefinitions(scroll.scrollId).getName()
						.toLowerCase() + "s.");
	}
