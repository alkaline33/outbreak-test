...
public class PestControl {
	...
	@Override
	public void run() {
		World.sendWorldMessage("THIS IS STILL BUGGED, DO NOT USE", false);
	    try {
			//World.sendWorldMessage("2lol2", false);
		...
		try {
		    ...
		    sendBeginningWave();
			 for (Player player : team) {
				    //	World.sendWorldMessage("3", false); issue
					player.getControlerManager().removeControlerWithoutCheck(); //issue is here
	...
	for (int trycount = 0; trycount < 10; trycount++) {
	    ...
	    if (World.isTileFree(baseTile.getPlane(), teleTile.getX(), teleTile.getY(), defs.size))
		break;
	    teleTile = baseTile;
	}
	...
	for (final Player player : team) {
		 final int zeal = (int) ((PestControlGame) player.getControlerManager().getControler()).getPoints();
	...
	if (knight.isDead())
	    ...
	    int commendation = data.getReward() * (Settings.getDropQuantityRate() + 1);
	    player.getDialogueManager().startDialogue("SimpleMessage",
	...
	for (Player player : team) {
		...
		if (npc != null) {
		    player.getPackets().sendIComponentText(408, count + 9, npc.getHitpoints() + "");
		    if (npc.isDead()) {
			player.getPackets().sendHideIComponent(408, count + 25, false);
			player.getPackets().sendHideIComponent(408, count+20, true);
		    }
		}
	    }
	...
	INTERMEDIATE(new int[] { /* Shifters */
		...
		3764, 3765, 3766, 3768, 3769 }, new int[] { 3734, 3735, 3736, 3737, 3738, 3739 }, 3),
