...
public class Lander {
	...
	public void run() {
	    try {
		if (seconds == 0 && byStanders.size() >= 1 || byStanders.size() >= AUTO_GAME)
		    begin();
		else if (seconds == 0)
		    seconds = LANDER_TIME;
	...
	if (playerList.size() > AUTO_GAME) {
		...
		player.getPackets().sendGameMessage("You have receivedpriority over other players.");
		playerList.remove(index);
	...
	player.getControlerManager().startControler("PestControlLobby", landerRequirement.ordinal());
	add(player);
	player.useStairs(-1, landerRequirement.getWorldTile(), 1, 2, "You board the lander.");
		// World.sendWorldMessage("1a", false);
	if (player.getCommendation() == 1000)
	    player.getPackets().sendGameMessage("Warning! Winning the upcomming game will not result in a reward. Please spend some Commendation.");
    }
	...
	INTERMEDIATE(70, new WorldTile(2641, 2644, 0), new WorldTile(2644, 2644, 0)),
	...
	private LanderRequirement(int requirement, WorldTile tile, WorldTile exit) {
