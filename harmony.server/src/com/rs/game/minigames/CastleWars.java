					...
					for (Player player : playing[i]
						...
						if (winner != -1) {
							if (winner == -2) {
								...
								player.getInventory().addItem(CW_TICKET, Settings.EVENT_SPOTLIGHT.contains("minigame") ? 2 : 1);
							} else if (winner == i) {
								player.getPackets().sendGameMessage("You won.");
								player.getInventory().addItem(CW_TICKET, Settings.EVENT_SPOTLIGHT.contains("minigame") ? 4 : 2);
							} else
								player.getPackets()
