				...
				if(stage == 0) {
					...
					player.getSkills().addXp(Skills.AGILITY, 15);
					 if (getStage(player) == 1) {
						...
						player.getInventory().addItem(995, 25000);
						player.getInventory().addItem(2996, Utils.random(1, 8));
						SeasonEvent.HandleActivity(player, "Agility", 15);
						player.lapsrun++;
						if (player.challengeid == 15 && player.challengeid > 0) {
							DailyChallenges.UpdateChallenge(player);
						}
						 if (player.AgilityTaskBarb > 0) {
							 player.AgilityTaskBarb -= 1;
						 }
			...
			public void run() {
				...
				if (stage == 3) {
					...
					player.getInventory().addItem(995, 10000);
						player.getInventory().addItem(2996, Utils.random(1, 5));
