...
public class Foods {
	...
	public static enum Food {
		...
		RAPTORBAGUETTE(28972, 60),
		
		ROCKCAKE(7509, -2),

		ROCKTAIL(15272, 23, 0, null, 10),
		
		ROCKTAIL_SOUP(29514, 25),
		
		PURPLE_SWEETS(10476, 20),
	...
	public static boolean eat(final Player player, Item item, int slot, final int itemId) {
		...
		if (itemId == 29636) {
		if (player.sheafeaten >= 10) {
			player.sendMessage("You cannot consume anymore Sheaf sharks today.");
			return false;
		}
		player.sheafeaten ++;
	}
		...
		if (itemId == 10476) {
			...
			player.heal(player.eventhealperk ? 220 : 200);
		} else {
			if (player.isSponsor() && Utils.random(1, 10) == 2) {
              ...
              .getItems()
              .set(slot,
                      food.getNewId() == 0 ? null : new Item(food.getNewId(), 1));
			}
			if (!player.isSponsor()) {
		...
		int hp = player.getHitpoints();
			player.heal(player.eventhealperk ? food.getHeal() * 11 : food.getHeal() * 10, player.eventhealperk ? food.getExtraHP() * 11 : food.getExtraHP() * 10);
