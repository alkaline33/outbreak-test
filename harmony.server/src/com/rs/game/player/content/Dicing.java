...
public class Dicing {
	...
	public static void handleRoll(final Player player, int itemId, boolean friends) {
		if (!player.isGambleHost() && player.getRights() < 2) {
	...
	public static void privateRoll(final Player player, final int itemId,
		...
		if (!World.DiceArea(player)) {
			player.sendMessage("You must be at ::dice to do this!");
			return;
		}
		if (!player.getInventory().containsItem(995, 1000000000)) {
			player.sendMessage("You need at least 1 billion coins in your inventory to do this!");
			return;
		}
		player.getPackets().sendGameMessage("Rolling...", true);
		player.getInventory().deleteItem(15098, 1);
		player.setNextAnimation(new Animation(11900));
		player.setNextGraphics(new Graphics(graphic));
		WorldTasksManager.schedule(new WorldTask() {
			@Override
			public void run() {
				int number = getRandom(lowest, highest);
				player.getInventory().addItem(15098, 1);
				player.setNextForceTalk(new ForceTalk("<img=3> I Rolled....  <col=00ff00>" +number + "</col>!"));
