...
import com.rs.game.world.timers.AntifireTimer;
import com.rs.game.world.timers.AntipoisonTimer;
import com.rs.game.world.timers.OverloadTimer;
import com.rs.game.world.timers.OverloadedTimer;
import com.rs.game.world.timers.PrayerRenewalTimer;
...
public final class Pots {
	...
	public static enum Pot {
		...
		S_SARADOMIN_BREW(new int[] { 29766, 29765, 29764, 29763 }, Effects.S_SARADOMIN_BREW),
		
		S_SARADOMIN_BREW_FLASK(new int[] { 29088,29090,29092,29094,29096,29098 }, Effects.S_SARADOMIN_BREW),
		...
		OVERLOADED(new int[] { 29633, 29632, 29631, 29630 }, Effects.OVERLOADED),
		
		OVERLOADED_FLASK(new int[] { 29075,29074,29073,29072,29071,29070 }, Effects.OVERLOADED),
		
		OVERPRAY(new int[] { 29770, 29769, 29768, 29767 }, Effects.OVERPRAY),
		
		OVERPRAY_FLASK(new int[] { 29068, 29066,29065,29064,29063,29062 }, Effects.OVERPRAY),
		
		EOVERLOAD(new int[] { 12140, 12142, 12144, 12146 }, Effects.EOVERLOAD),
		
		EOVERLOAD_FLASK(new int[] { 29076, 29078,29080,29082,29084,29086 }, Effects.EOVERLOAD),
		
		SUPREMEOVERLOAD(new int[] { 33210, 33208, 33206, 33204, 33202,33200 }, Effects.EOVERLOAD),

		ANTI_FIRE(new int[] { 2452, 2454, 2456, 2458 }, Effects.ANTI_FIRE),
		
		SUPER_ANTI_FIRE(new int[] { 15304, 15305, 15306, 15307 }, Effects.ANTI_FIRE),
		...
		SUPER_ANTIFIRE_FLASK(new int[] { 23489, 23490, 23491, 23492, 23493, 23494 }, Effects.ANTI_FIRE),
	...
	public enum Effects {
		...
		ANTIPOISON() {
			...
			public void extra(Player player) {
				player.addPoisonImmune(180000);
				AntipoisonTimer.addTime(player, 86);
		...
		SUPER_ANTIPOISON() {
			...
			public void extra(Player player) {
				...
				AntipoisonTimer.addTime(player, 346);
				player.getPackets().sendGameMessage(
						"You are now immune to poison.");
			}
		},
		...
		SUPER_ENERGY() {
	     ...
	     ANTI_FIRE() {
            ...
            public void extra(final Player player) {
				AntifireTimer.addTime(player, 360);
		...
		SUPER_ANTI_FIRE() {
			@Override
			public void extra(final Player player) {
				AntifireTimer.addTime(player, 420);
			player.addFireImmune(420000);
			final long current = player.getFireImmune();
			player.getPackets().sendGameMessage( "By drinking your super antifire, you are now immune to dragonfire.");
			WorldTasksManager.schedule(new WorldTask() {
			boolean stop = false;
			@Override
			public void run() {
			...
			if (!stop) {
			player.getPackets().sendGameMessage("<col=480000>Y our super antifire potion is about to run out...</col>");
			stop = true;
			} else {
			stop();
			player.getPackets().sendGameMessage("<col=480000>Y our super antifire potion has ran out...</col>");
			}
			}
			}, 500, 100);
			}
		...
		PRAYER_POTION() {
			...
			public void extra(Player player) {
				for (int bosspet : Settings.PETS_WITH_PERKS) {
					if (player.getPetManager().getNpcId() == bosspet && player.getPetPerk().getContainer().contains(new Item(28916))) {
						player.getPrayer()
						.restorePrayer(
								(int) ((int) (Math.floor(player.getSkills()
										.getLevelForXp(Skills.PRAYER) * 0.625) + 17.5) * player
										.getAuraManager()
										.getPrayerPotsRestoreMultiplier()));
					} 
				}
			...
			public void extra(Player player) {
				for (int bosspet : Settings.PETS_WITH_PERKS) {
					if (player.getPetManager().getNpcId() == bosspet && player.getPetPerk().getContainer().contains(new Item(28916))) {
						...
						.restorePrayer(
								(int) ((int) (Math.floor(player.getSkills()
										.getLevelForXp(Skills.PRAYER) * 0.75) + 17.5) * player
										.getAuraManager()
										.getPrayerPotsRestoreMultiplier()));
					} 
				}
				player.getPrayer()
						.restorePrayer(
								(int) ((int) (Math.floor(player.getSkills()
										.getLevelForXp(Skills.PRAYER) * 3) + 70) * player
										.getAuraManager()
										.getPrayerPotsRestoreMultiplier()));
			}
		},
		COMBAT_POTION(Skills.STRENGTH, Skills.ATTACK) {
			@Override
			public int getAffectedSkill(Player player, int skillId,
					int actualLevel, int realLevel) {
				int level = actualLevel > realLevel ? realLevel : actualLevel;
		...
		EXTREME_DEF_POTION(Skills.DEFENCE) {
			...
			@Override
			public boolean canDrink(Player player) {
				if ( player.getControlerManager().getControler() instanceof CrucibleControler || FfaZone.isOverloadChanged(player)) {
					player.getPackets().sendGameMessage(
							"You cannot drink this potion here.");
					return false;
				}
		...
		S_SARADOMIN_BREW("You drink some of the foul liquid.", Skills.ATTACK,
				Skills.DEFENCE, Skills.STRENGTH, Skills.MAGIC, Skills.RANGE) {

			@Override
			public boolean canDrink(Player player) {
				if (!player.canmakesupersarabrew) {
					player.sendMessage("You must unlock the Super Saradomin brew recipe from Calamity first!");
					return false;
				}
				return true;
			}
			...
			@Override
			public void extra(Player player) {
				int hitpointsModification = (int) (player.getMaxHitpoints() * 0.20);
				player.heal(hitpointsModification + 25, hitpointsModification);
			}
		},
		...
		OVERLOADED() {
			...
			public boolean canDrink(Player player) {
				...
				if (player.getHitpoints() <= 500 || player.getOverloadDelay() > 960) {
					player.getPackets()
					.sendGameMessage(
							"You need more than 500 life points to survive the power of overloaded.");
					return false;
				}
				return true;
			}
			...
			@Override
			public void extra(final Player player) {
				OverloadedTimer.addTime(player);
				if (player.potionperk == true) {
					player.setOverloadDelay(2004);
				} else {
				player.setOverloadDelay(1002);
				}
				WorldTasksManager.schedule(new WorldTask() {
		...
		EOVERLOAD() {
			...
			public boolean canDrink(Player player) {
				...
				if (!player.isExtremeDonator()) {
					player.sendMessage("You must be an extreme donator to use this potion.");
					return false;
				}
				if (player.getHitpoints() <= 500 || player.getOverkillDelay() > 480) {
					player.getPackets()
					.sendGameMessage(
							"You need more than 500 life points to survive the power of overkill.");
					return false;
				}
				return true;
			}
			...
			public void extra(final Player player) {
				...
				if (World.Level3Zone(player)) {
					player.setOverkillDelay(3004);
				} else if (player.potionperk == true) {
		...
		OVERPRAY() {
			...
			public boolean canDrink(Player player) {
				...
				if (!player.canmakeoverpray) {
					player.sendMessage("You must have unlocked the Overpray recipe from Calamity to use this.");
					return false;
				}
				/*		if (player.getOverloadDelay() > 0) {
					player.getPackets().sendGameMessage(
				...
				if (player.getHitpoints() <= 500 || player.getOverloadDelay() > 480) {
					player.getPackets()
					.sendGameMessage(
							"You need more than 500 life points to survive the power of overpray.");
					return false;
				}
				return true;
			}
			...
			public void extra(final Player player) {
				...
				if (player.potionperk == true) {
					player.setOverloadDelay(1002);
					player.setPrayerRenewalDelay(1002);
				} else {
					player.setOverloadDelay(501);
		...
		PRAYER_RENEWAL() {
			@Override
			public void extra(Player player) {
				PrayerRenewalTimer.addTime(player);
				if (player.potionperk == true) {
					player.setPrayerRenewalDelay(1002);
				} else {
					player.setPrayerRenewalDelay(501);
			...
			public void extra(Player player) {
				for (int bosspet : Settings.PETS_WITH_PERKS) {
					if (player.getPetManager().getNpcId() == bosspet && player.getPetPerk().getContainer().contains(new Item(28916))) {
						...
						.restorePrayer(
								(int) ((int) (Math.floor(player.getSkills()
										.getLevelForXp(Skills.PRAYER) * 0.0825) + 2.5) * player
										.getAuraManager()
										.getPrayerPotsRestoreMultiplier()));
					} 
				}
	...
	public static boolean pot(Player player, Item item, int slot) {
		...
		player.getPackets().sendGameMessage(
				dosesLeft == 0 ? "You have finished your " + (name.contains("flask") ? "flask and the glass shatters to pieces." : "potion.") : "You have "
	...
	public static void resetEOverLoadEffect(Player player) {
		if (!player.isDead()) {
			int actualLevel = player.getSkills().getLevel(Skills.ATTACK);
			int realLevel = player.getSkills().getLevelForXp(Skills.ATTACK);
			if (actualLevel > realLevel) {
		...
		player.setOverkillDelay(0);
		player.getPackets().sendGameMessage(
				"<col=480000>The effects of overkill potion have worn off and you feel normal again.");
	}
	
	public static void applyEOverLoadEffect(Player player) {
		if (player.getControlerManager().getControler() instanceof Wilderness
			...
			realLevel = player.getSkills().getLevelForXp(Skills.MAGIC);
			level = actualLevel > realLevel ? realLevel : actualLevel;
			player.getSkills().set(Skills.MAGIC, level + 10);

			actualLevel = player.getSkills().getLevel(Skills.RANGE);
			realLevel = player.getSkills().getLevelForXp(Skills.RANGE);
			level = actualLevel > realLevel ? realLevel : actualLevel;
			player.getSkills().set(Skills.RANGE,
					(int) (level + 4 + Math.floor(realLevel / 4.2)));
		}
	}
