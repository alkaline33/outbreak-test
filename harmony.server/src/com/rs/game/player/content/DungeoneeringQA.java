...
public class DungeoneeringQA {
	...
	public static void Run() {
		...
		for(Player participant : World.getPlayers()) {
			...
			if(TriviaArea(participant)) {
				participant.getPackets().sendGameMessage("[Dungeoneering]"+questions[rand][0]);
			}
		}
	}
	...
	public static void sendRoundWinner(String winner) {
		for(Player participant : World.getPlayers()) {
			...
			if(TriviaArea(participant)) {
				victory = true;
				participant.getPackets().sendGameMessage("[Dungeoneering]Congratulations, "+winner+" won round "+round+".");
			}
		}
	}
	...
	public static boolean TriviaArea(final Player participant) {
		if(participant.getX() >= 1 && participant.getX() <= 10000 && participant.getY() >= 1 && participant.getY() <= 10000) {
			return true;
		}
		return false;
	}
