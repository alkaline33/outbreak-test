...
public class AuraManager implements Serializable {
	...
	public void activate() {
		...
		//		return;
		//	}
			activation = player.superchargedperk ? Utils.currentTimeMillis() + getActivationTime(item.getId()) * 1000 * 2 : Utils.currentTimeMillis() + getActivationTime(item.getId()) * 1000;
	...
	public void sendAuraRemainingTime() {
		if (!isActivated()) {
			...
			player.getPackets().sendGameMessage(
					"Currently deactive. It is ready to use.");
			return;
		}
		player.getPackets().sendGameMessage(
 ...
 */
	...
	public double getBerserkerBoost(boolean defenceBoost) {
		if (!isActivated() || World.isPvpArea(player)) {
			return 1;
		}
		return player.getEquipment().getAuraId() != 22897 ? 1 : defenceBoost ? 0.85 : 1.1;
	}
	...
	/*public boolean usingSantaBliss() {
		if (!isActivated())
			return false;
		int aura = player.getEquipment().getAuraId();
		return aura == 22893;
