		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (i.getId() != herb.getHerbId())
					return;
				if (player.challengeid == 16 && herb.getHerbId() == 207 && player.challengeamount > 0) {
					DailyChallenges.UpdateChallenge(player);
				}
				if (player.challengeid == 17 && herb.getHerbId() == 2485 && player.challengeamount > 0) {
					DailyChallenges.UpdateChallenge(player);
				}
				if (player.challengeid == 18 && herb.getHerbId() == 219 && player.challengeamount > 0) {
					DailyChallenges.UpdateChallenge(player);
				}
