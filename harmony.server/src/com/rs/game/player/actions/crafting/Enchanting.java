...
public class Enchanting {
    ...
    public enum Types {
        SAPPHIRE(1, 7, 2550, 3853, 1727, 11074, 9240),
        EMERALD(2, 27, 2552, 5521, 1729, 11079, 9241),
        RUBY(3, 49, 2568, 11194, 1725, 11088, 9242),
        DIAMOND(4, 57, 2570, 11090, 1731, 11095, 9243),
        DRAGONSTONE(5, 86, 2572, 11105, 1712, 11188, 9244),
        ONYX(6, 87, 6583, 11128, 6585, 11133, 9245);
        
        int enchantLevel, level, ring, necklace, amulet, bracelet, bolt;
        
        Types(int enchantLevel, int level, int ring, int necklace, int amulet, int bracelet, int bolt) {
    ...
    public static boolean startEnchant(Player player, int itemId, int enchantLevel) {
        ...
        for (Types t : Types.values()) {
            if (enchantLevel == t.enchantLevel) {
                ...
                if (itemId == base.baseBolt) {
                    return enchant(player, t.level, base.baseBolt, t.bolt, enchantLevel);
                }
            }
        }
        return false;
    ...
    public static boolean enchant(Player player, int reqLevel, int toEnchant, int toMake, int enchantLevel) {
        ...
        if (ItemDefinitions.getItemDefinitions(toMake).isStackable() && player.getInventory().containsItem(toEnchant, 10)) {
        player.getInventory().addItem(toMake, 10);
        player.getInventory().deleteItem(toEnchant, 10);
        } else {
    ...
    public enum Base {
       ...
       SAPPHIRE(1, 1637, 1656, 1694, 11072, 9337),
       EMERALD(2, 1639, 1658, 1696, 11076, 9338),
       RUBY(3, 1641, 1660, 1698, 11085, 9339),
       DIAMOND(4, 1643, 1662, 1700, 11092, 9340),
       DRAGONSTONE(5, 1645, -1, 1702, -1, 9341),
       ONYX(6, 6575, 6577, 6581, 11130, 9342);
        
        int level, baseRing, baseNecklace, baseAmulet, baseBracelet, baseBolt;
        Base(int level, int baseRing, int baseNecklace, int baseAmulet, int baseBracelet, int baseBolt) {
            this.level = level;
    ...
    public static boolean canBeEnchanted(Base b, int itemId) {
        return b.baseAmulet == itemId || b.baseNecklace == itemId || b.baseRing == itemId || b.baseBracelet == itemId || b.baseBolt == itemId;
    }
