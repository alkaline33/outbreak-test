...
public class Spinning {
	 ...
	 *
	 * @param afterId
	 *            The after ID to be dependant on.
	 ...
	 *
	 * @param player
	 *            The player spinning the Item.
	 * @param item
	 *            The SpinningItem being spun.
	 */
	private static void spin(Player player, SpinningItem item, int itemId) {
		if (player.getSkills().getLevel(Skills.CRAFTING) < item.getSkillRequirement()) {
			player.getPackets()
					.sendGameMessage("You need a Crafting level of " + item.getSkillRequirement() + " to craft this.");
			return;
		}
		player.setNextAnimationNoPriority(SPINNING_ANIMATION);
	...
	public enum SpinningItem {
		 ...
		 * @param exp
		 *            The experience gained for spinning the item.
		 */
