...
public class Fletching extends Action {
	...
	public static enum Fletch {
		...
		MAPLE_BOW(1517, 946, new int[] { 64, 62, 9448 },
				new int[] { 11, 55, 54 }, new double[] { 50, 58.3, 32 },
				new Animation(6702)),
				
		DREAM_LOG(9067, 946, new int[] { 995 },
				new int[] { 90, 90, 90 }, new double[] { 140 },
				new Animation(6702)),
		...
		REDWOOD_SHIELD(29138, 946, new int[] { 29136 }, new int[] { 92 },
				new double[] { 216 }, new Animation(6702)),
		
		/**
		 * Stocks
		 */
		MAHOGANY_STOCK(6332, 946, new int[] {  9450 } , new int[] { 61 },  
				new double [] { 78.25 }, new Animation(6702)),
		TEAK_STOCK(6333, 946, new int[] {  9446 } , new int[] { 46 },  
				new double [] { 75.25 }, new Animation(6702)),
		...
		HYDRIX_BOLTS(31867, 28465, new int[] { 29616 }, new int[] { 80 },
				new double[] { 10.2 }, new Animation(-1)),
		
		ASCENSION_BOLTS(28436, 1755, new int[] { 28465 }, new int[] { 90 },
				new double[] { 20 }, new Animation(-1)),
	...
	public boolean process(Player player) {
		...
		if (fletch.getSelected() != 946 && !player.getInventory().containsItem(fletch.getSelected(), 1)) {
			return false;
		}
	...
	public static boolean maxMakeQuantityTen(Item item) {
		if (item.getId() == 52 || ItemDefinitions.getItemDefinitions(item.getId()).getName().toLowerCase().contains("dart") || ItemDefinitions.getItemDefinitions(item.getId()).getName().toLowerCase().contains("shaft") || item.getId() == 53 || ItemDefinitions.getItemDefinitions(item.getId()).getName().toLowerCase().contains("bolt") || ItemDefinitions.getItemDefinitions(item.getId()).getName().toLowerCase().contains("arrow") || item.getId() == 314) {
			return true;
		}
		return false;
	}
	...
	public int processWithDelay(Player player) {
		ticks--;
		int amount = maxMakeQuantityTen(new Item(fletch.getProduct()[option])) ? 15 : 1;
		// if (fletch.
		if (amount == 15) {
			if (player.getInventory().getNumerOf(fletch.getSelected()) < 15 && fletch.getId() != 1511) {
				amount = player.getInventory().getNumerOf(fletch.getSelected());
			}
			if (player.getInventory().getNumerOf(fletch.getId()) < 15 && fletch.getId() != 1511) {
				amount = player.getInventory().getNumerOf(fletch.getId());
			}
		...
		if (Utils.random(1, 10) == 1 && player.getEquipment().getCapeId() == 29988) {
			player.getBank().addItem(fletch.getProduct()[option], amount, true);
			player.sendMessage("Your fletching master cape perk has added extra products to your bank.");
		} else if (Utils.random(1, 10) == 1 && player.getEquipment().getCapeId() == 20771 && player.getSkills().getXp(Skills.FLETCHING) >= 104273167) {
			player.getBank().addItem(fletch.getProduct()[option], amount, true);
			player.sendMessage("Your fletching master cape perk has added extra products to your bank.");
		}
		...
		if (player.challengeid == 25 && player.challengeamount > 0 && fletch.getProduct()[option] == 845) {
			DailyChallenges.UpdateChallenge(player);
		}
		if (player.challengeid == 26 && player.challengeamount > 0 && fletch.getProduct()[option] == 855) {
			DailyChallenges.UpdateChallenge(player);
		}
		if (player.challengeid == 27 && player.challengeamount > 0 && fletch.getProduct()[option] == 859) {
			DailyChallenges.UpdateChallenge(player);
		}
		player.getPackets().sendGameMessage(
				"You attempt to create a "
