...
public class Summoning implements Serializable {
    ...
    public static final class Pouches implements Serializable {
        ...
        public static Pouches forId(int id) {
            return pouches.get(Integer.valueOf(id));
        }
...
/*		if (player.ironman == true) {
			player.sendMessage("Ironman accounts cannot use familiars.");
		}*/
		if (player.getControlerManager().getControler() instanceof CalamityPlaying || World.TheCalamity(player) || World.HydraBossRoom(player)) {
			player.getPackets().sendGameMessage("You cannot spawn familiars in here.");
			return;
		}
    ...
    public static void createPouch(final Player player, int itemId, int amount) {
        ...
        for (i = 0; i < amount; i++) {
			...
			// double multiplier = 3;
			if (Utils.random(1, 10) == 1 && player.getEquipment().getCapeId() == 29976) {
				player.sendMessage("Your summoning master cape perk has saved some supplies.");
			} else	if (Utils.random(1, 10) == 1 && player.getEquipment().getCapeId() == 20771 && player.getSkills().getXp(Skills.SUMMONING) >= 104273167) {
				player.sendMessage("Your summoning master cape perk has saved some supplies.");
			} else {
            player.getInventory().removeItems(pouch.getItems());
			}
            player.getInventory().addItem(pouch.getPouchId(), 1);
			player.getSkills().addXp(Skills.SUMMONING, pouch.getCreateExperience());
            player.getPackets().sendObjectAnimation(
                    new WorldObject(28734, 10, 1, 2209, 5344, 0),
                    new Animation(8509));
        }
        if (i == 1) {
