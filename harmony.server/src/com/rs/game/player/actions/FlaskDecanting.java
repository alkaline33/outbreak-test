...
public class FlaskDecanting {
	...
	public static Pot getFlask(Pot p) {
		for (Pot pot : Pot.values()) {
			if (pot.id.length < 6) {
				continue;
			}
			if (pot.getEffect().equals(p.getEffect())) {
				return pot;
			}
		}
		return null;
	...
	public static boolean mixPot(Player player, Item fromItem, Item toItem, int fromSlot, int toSlot) {
		...
		if (fromItem.getId() == FLASK || toItem.getId() == FLASK) {
			Pot pot = getPot(fromItem.getId() == FLASK ? toItem.getId() : fromItem.getId());
			...
			int doses = getDoses(pot, fromItem.getId() == FLASK ? toItem : fromItem);
			if (doses == 1) {
			...
			doses -= vialDoses;
			player.getInventory().getItems().set(fromItem.getId() == FLASK ? toSlot : fromSlot, new Item(pot.getIdForDoses(doses), 1));
			player.getInventory().getItems().set(fromItem.getId() == FLASK ? fromSlot : toSlot, new Item(pot.getIdForDoses(vialDoses), 1));
