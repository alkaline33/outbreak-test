...
public class Prayer implements Serializable {
	...
	private final static double[][] prayerDrainRate = {
			{ 1.2, 1.2, 1.2, 1.2, 1.2, 0.6, 0.6, 0.6, 3.6, 1.8, 1.8, 0.6, 0.6, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3, 1.2, 0.6, 0.18, 0.18, 0.24, 0.15, 0.2, 0.18 }, { 1.8, 0.24, 0.24, 0.24, 0.24, 1.8, 0.3, 0.3, 0.3, 0.3, 0.36, 0.36, 0.36, 0.36, 0.36, 0.36, 0.36, 1.2, 0.2, 0.2 } };
	...
	public double getMageMultiplier() {
		...
		if (usingPrayer(0, 4)) {
			...
			value += d / 100;
		}else if (usingPrayer(1, 19) && player.turmoildirect == 1) {
			double d = (player.mageturmoilupgrade ? 20 : 15) + leechBonuses[5];
			value += d / 100;
		}
		return value;
	}
	...
	public double getRangeMultiplier() {
		...
		if (usingPrayer(0, 3)) {
			...
			value += d / 100;
		} else if (usingPrayer(1, 19) && player.turmoildirect == 2) {
			double d =  (player.rangeturmoilupgrade ? 20 : 15) + leechBonuses[4];
			value += d / 100;
		}
		return value;
	}
	...
	public double getAttackMultiplier() {
		...
		if (usingPrayer(0, 2)) {
			...
			value += d / 100;
		} else if (usingPrayer(1, 19) && player.turmoildirect == 0) {
			double d =  (player.meleeturmoilupgrade ? 20 : 15) + leechBonuses[8];
			value += d / 100;
		}
		return value;
	}
	...
	public double getStrengthMultiplier() {
		...
		if (usingPrayer(0, 1)) {
			...
			double d = (player.meleeturmoilupgrade ? 28 : 23) + leechBonuses[10];
			value += d / 100;
		}
		return value;
	}
	...
	public double getDefenceMultiplier() {
		...
		if (usingPrayer(0, 0)) {
			...
			value += d / 100;
		} else if (usingPrayer(1, 19)) {//turmoil
	...
	public void increaseTurmoilBonus(NPC npc) {
		if (npc.getBonuses() != null) {
			int defenceLevel = npc.getBonuses()[0] / 5;
			int attackLevel = npc.getBonuses()[5] / 5;
			//System.out.println("levels: " + defenceLevel + ", " + attackLevel);
			// int strengthLevel = npc.getBonuses()[5] / 5;
			leechBonuses[8] = (int) (100 * Math.floor(0.15 * attackLevel) / attackLevel);
			leechBonuses[9] = (int) (100 * Math.floor(0.15 * defenceLevel) / defenceLevel);
			leechBonuses[10] = (int) (100 * Math.floor(0.1 * attackLevel) / attackLevel);
			//System.out.println("Adjust stats: " + leechBonuses[8] + ", " + leechBonuses[10] + ", " + leechBonuses[9]);
			adjustStat(0, leechBonuses[8]);
			adjustStat(1, leechBonuses[10]);
			adjustStat(2, leechBonuses[9]);
		} else {
			leechBonuses[8] = (int) (100 * Math.floor(0.15 * 80) / 80);
			leechBonuses[9] = (int) (100 * Math.floor(0.15 * 80) / 80);
			leechBonuses[10] = (int) (100 * Math.floor(0.1 * 80) / 80);
			adjustStat(0, leechBonuses[8]);
			adjustStat(1, leechBonuses[10]);
			adjustStat(2, leechBonuses[9]);
		}
	...
	private void adjustStat(int stat, int percentage) {
	...
	private void closePrayers(int prayerId) {
		if (ancientcurses) {
			if (prayerId == 1) {
				if (leechBonuses[0] > 0) {
					player.sendMessage("Your Attack is now unaffected by sap and leech curses.");
				}
				adjustStat(0, 0);
				adjustStat(1, 0);
				adjustStat(2, 0);
				...
				if (leechBonuses[1] > 0) {
					player.sendMessage("Your Range is now unaffected by sap and leech curses.");
				}
				adjustStat(2, 0);
				adjustStat(4, 0);
				leechBonuses[1] = 0;
				...
				if (leechBonuses[2] > 0) {
					player.sendMessage("Your Magic is now unaffected by sap and leech curses.");
				}
				adjustStat(2, 0);
				adjustStat(5, 0);
				leechBonuses[2] = 0;
				...
				if (leechBonuses[4] > 0) {
					player.sendMessage("Your Ranged is now unaffected by sap and leech curses.");
				}
				adjustStat(4, 0);
				leechBonuses[4] = 0;
			} else if (prayerId == 12) {
				...
				if (leechBonuses[6] > 0) {
					player.sendMessage("Your Defence is now unaffected by sap and leech curses.");
				}
				adjustStat(2, 0);
				leechBonuses[6] = 0;
			} else if (prayerId == 14) {
				if (leechBonuses[7] > 0) {
					player.sendMessage("Your Strength is now unaffected by sap and leech curses.");
				}
				adjustStat(1, 0);
				leechBonuses[7] = 0;
			} else if (prayerId == 19) {
		...
		if (hasPrayersOn()) {
			...
			if (player.getCurrentFriendChat() != null) {
				...
				if (war != null && war.get(Rules.NO_PRAYER) && (war.getFirstPlayers().contains(player) || war.getSecondPlayers().contains(player))) {
					player.sendMessage("Prayer has been disabled during this war.");
					return;
				}
			}
			boolean hasOn = false;
	...
	private boolean usePrayer(int prayerId) {
		...
		if (player.getSkills().getLevelForXp(5) < prayerLvls[getPrayerBook()][prayerId]) {
			player.sendMessage("You need a prayer level of at least " + prayerLvls[getPrayerBook()][prayerId] + " to use this prayer.");
			return false;
		}
		if (getPrayerBook() == 0 && prayerId == 27) {
			...
			if (getPrayerBook() == 0 && prayerId == 25) {
				if (player.getSkills().getLevelForXp(Skills.DEFENCE) < 65) {
			...
			if (player.getSkills().getLevelForXp(Skills.DEFENCE) < 30) {
				player.sendMessage("You need a defence level of at least 30 to use this prayer.");
				return false;
			}
		}
		...
		if (getPrayerBook() == 0) {
			...
			case 16:
				...
				break;
			case 19://turmoil
				String style = player.turmoildirect == 0 ? "<col=ff0000>Melee</col>" : player.turmoildirect == 1 ? "<col=0e5bf6>Magic</col>" : "<col=00ff00>Range</col>";
				player.getPackets().sendGameMessage(Colors.gray+"Your turmoil is currently directing its power to "+style+" style");
				if (!usingQuickPrayer) {
					player.setNextAnimation(new Animation(12565));
					player.setNextGraphics(new Graphics(2226));
				}
				if (player.turmoildirect == 0 && player.meleeturmoilupgrade) {
					player.setNextGraphics(new Graphics(2451));
				} else if (player.turmoildirect == 1 && player.mageturmoilupgrade) {
					player.setNextGraphics(new Graphics(2451));
				} else if (player.turmoildirect == 2 && player.rangeturmoilupgrade) {
					player.setNextGraphics(new Graphics(2451));
				}
				closePrayers(closePrayers[getPrayerBook()][1],
	...
	public void processPrayerDrain() {
		...
		for (int index = 0; index < onPrayers[getPrayerBook()].length; index++) {
			if (onPrayers[getPrayerBook()][index]) {
				...
				if (drainTimer != 0 && drainTimer <= currentTime) {
					...
					// if (player.getPerksManager().hasPerk(Perks.ZAROSIAN_DECENDANT))
					// rate *= 1.2;
					if (player.prayerperk) {
						rate *= 2;
					}
	...
	public boolean usingPrayer(int prayerId) {
		return usingPrayer(getPrayerBook(), prayerId);
	}
	...
	public boolean canUsePrayer(int prayerId) {
		if (player.getSkills().getLevelForXp(5) < prayerLvls[getPrayerBook()][prayerId]) {
			return false;
		}
	...
	public boolean hasFullPrayerpoints() {
		for (int bosspet : Settings.PETS_WITH_PERKS) {
			if (player.getPetManager().getNpcId() == bosspet && player.getPetPerk().getContainer().contains(new Item(29121))) {
				return getPrayerpoints() >= player.getSkills().getLevelForXp(Skills.PRAYER) * 10 + 200;
			}
		}
	...
	public int maxPrayerPoints() {
		for (int bosspet : Settings.PETS_WITH_PERKS) {
			if (player.getPetManager().getNpcId() == bosspet && player.getPetPerk().getContainer().contains(new Item(29121))) {
				return player.getSkills().getLevelForXp(Skills.PRAYER) * 10 + 200;
			}
		}
