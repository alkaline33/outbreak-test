...
public final class EmotesManager implements Serializable {
	...
	public void useBookEmote(int id) {
		...
		if (!unlockedEmotes.contains(id) && player.getRights() < 2) {
			if (!unlockedEmotes.contains(109) || !unlockedEmotes.contains(112)) {
			unlockedEmotes.add(109); // backflip
			unlockedEmotes.add(112); // breakdance
			}
			if (id == 41)
			...
			if (id == 52) {// Seal Of Approval
				...
				if (player.breakdanceemoteunlocked != true) {
					player.sendMessage("You must unlock this via the book of movements first.");
					return;
				}
