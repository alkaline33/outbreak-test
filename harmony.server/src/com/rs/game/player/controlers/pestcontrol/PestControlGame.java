...
public class PestControlGame extends Controler {
	...
	refreshDamagePoints();
	player.getInterfaceManager().sendOverlay(408, true);
    }

    private void refreshDamagePoints() {
	boolean isGreen = getPoints() > 5000;
	player.getPackets().sendIComponentText(408, 7, (isGreen ? "<col=75AE49>" : "") + (int) getPoints() + "</col>");
    }
	...
	World.sendWorldMessage("3o", false);
	player.getInterfaceManager().removeInterface(408, 0);
	player.reset();
    }
	...
	player.useStairs(-1, new WorldTile(2657, 2639, 0), 1, 2);
	World.sendWorldMessage("1o", false);
	return true;
    }
	...
	if (control != null) {
	    ...
	    World.sendWorldMessage("2o", false);
	    control.getPlayers().remove(player);
	}
	return true;
    }
