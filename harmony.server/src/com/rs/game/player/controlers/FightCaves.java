...
public class FightCaves extends Controler {
	...
	public static void enterFightCaves(Player player) {
		...
		if (player.completedFightCaves) {
			player.getControlerManager().startControler("FightCavesControler", 63); // start at wave
		} else {
	...
	public void exitCave(int type) {
		...
		if (type == 0 || type == 2) {
			...
			if(type == 1 || type == 4) {
				...
				//	UpdateActivities.Activities(player, null , 2, 1, 0);
				//	SeasonEvent.HandleActivity(player, "Jad", 0);
						World.sendWorldMessage("<img=6><col=993300>News: "+ player.getDisplayName() + " has just completed the Fight Caves!", false);
