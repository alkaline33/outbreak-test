...
42 42 maxX, maxY
	...
	public boolean processMagicTeleport(WorldTile toTile) {
		player.getPackets().sendGameMessage("You can't teleport out of here!");
		return false;
	}
		...
		if(type == 0) {
			...
			if(type == 1 || type == 4) {
				...
				if(type == 4) {
					...
					//UpdateActivities.Activities(player, null , 2, 0, 0);
					Integer reward = (Integer) player.getTemporaryAttributtes().get("FightKilnReward");
					int itemId = reward != null &&  reward == 1 ? 21512 : 23659;
					if(!player.getInventory().addItem(itemId, 1)) {
						World.addGroundItem(new Item(itemId, 1), new WorldTile(player), player, true, 180,true);
					}
					player.kilnruns ++;
					player.sendMessage("You have completed "+player.kilnruns+" fight kilns.");
					World.sendWorldMessage("<img=6><col=660099>News: "+player.getDisplayName()+" has completed the Fight Kiln! (KC #"+player.kilnruns+")", false);
					player.reset();
				}
			}
			player.getInventory().removeItems(new Item(23653, Integer.MAX_VALUE)
