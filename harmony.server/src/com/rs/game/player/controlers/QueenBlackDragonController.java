				...
				try {
					...
					FadingScreen.unfade(player, time, new Runnable() {
						...
						public void run() {
							...
							player.getInterfaceManager().sendTab(player.getInterfaceManager().hasRezizableScreen() ? 1 : 0, 1285);//1133);
	...
	public boolean processObjectClick1(WorldObject object) {
		...
		if (object.getId() == 70790) {
			...
			FadingScreen.fade(player, new Runnable() {
				...
				//	UpdateActivities.Activities(player, null , 13, 0, 0);
		...
		if (object.getId() == 70813) {
			Magic.sendObjectTeleportSpell(player, true, new WorldTile(1199, 6499, 0));
			return false;
		}
	...
	public void process() {
		...
		if (player.getY() < base.getY() + 28) {
			...
			if (platformStand++ == 5) {
				...
				player.applyHit(new Hit(npc, 150, HitLook.REGULAR_DAMAGE));
				platformStand = 0;
			}
		} else {
			platformStand = 0;
