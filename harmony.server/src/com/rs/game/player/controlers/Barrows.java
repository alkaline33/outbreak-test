...
public final class Barrows extends Controler {
	...
	public void sendReward() {
		...
		if (percentage == 0) {
			player.sendMessage("You cannot loot this chest without killing a brother!");
			return;
		}
		percentage += player.getBarrowsKillCount() / 10;
		if(percentage >= Math.random()*100) {
			//reard barrows
			player.getInventory().addItem(BARROW_REWARDS[Utils.random(BARROW_REWARDS.length)]);
		}
		if(Utils.random(10) == 0) {
			player.getInventory().addItem(RARE_REWARDS[Utils.random(RARE_REWARDS.length)]);
		}
		if(Utils.random(3) != 0)
		 {
			player.getInventory().addItem(COMMUM_REWARDS[Utils.random(COMMUM_REWARDS.length)]);
		//here reward other stuff normaly
		}
		//SeasonEvent.HandleActivity(player, "Barrows", 0);
		player.barrowchestloot++;
		//player.dailybarrowschest++;
		player.getInventory().addItem(new Item(995, Utils.random(10000, 80000))); // money reward at least always
		/*if (player.dailybarrowschest == 3) {
			player.sendMessage(Colors.lightGray + "<img=6>You have completed the Daily Money Maker: Loot 3 Barrows Chests! Your rewards have been placed in your bank.");
			player.getBank().addItem(29541, 1, true);
			player.getBank().addItem(6199, 1, true);
		}*/
		player.sendMessage(Colors.orange+"You have looted a total of "+player.barrowchestloot+" barrows chests!");
		//Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(3565, 3313, 0));
	}
