...
public class ChargesManager implements Serializable {
	...
	public void process() {
		...
		for (int slot = 0; slot < items.length; slot++) {
			...
			if (item == null|| player.nodegradeperk == true && !item.getName().contains("Lucky")) {
				continue;
			}
			if (player.getAttackedByDelay() > Utils.currentTimeMillis()) {
				...
				if (newId != -1) {
					...
					player.getPackets().sendGameMessage(
							""+Colors.red+"Your " + item.getDefinitions().getName()
									+ " degraded.");
				}
			}
			int defaultCharges = ItemConstants.getItemDefaultCharges(item
					.getId());
			if (defaultCharges == -1  || player.nodegradeperk == true&& !item.getName().contains("Lucky")) {
				continue;
			}
			if (ItemConstants.itemDegradesWhileWearing(item.getId())) {
			 ...
			 {
				degrade(item.getId(), defaultCharges, slot);
			//World.sendWorldMessage(""+degrade, false);
			}
		}
	}
	...
	public void wear(int slot) {
		...
		if (newId == -1|| player.nodegradeperk == true&& !item.getName().contains("Lucky")) {
			return;
		}
		...
		player.getAppearence().generateAppearenceData();
		player.getPackets().sendGameMessage(
				""+Colors.red+"Your " + item.getDefinitions().getName() + " degraded.");
	}
