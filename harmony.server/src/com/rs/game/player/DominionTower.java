...
public final class DominionTower implements Serializable {
	...
	private static final Boss[] BOSSES = { new Boss("Elvarg", "Grrrr", 14548),
			...
			new Boss("Glacor", "Breezeeeeeeee", 14301),
			new Boss("Tectonic the Beast", "Gg m8, u r getin rekt m8", 10106),
			new Boss("Dryax", "Good to see you old friend.", 15174),
			new Boss("Avatar of Destruction", "I'm going to crush you!!!!", 8596),
			new Boss("Bork", "Well, well, well...", 7133),
			new Boss("Elvarg", "Burn!!!!", 742),
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (count == 0) {
					...
					//player.getInventory().addItem(BOSSES[index].item, 1);
					player.closeInterfaces();
	...
	public void win(int mode) {
		...
		SeasonEvent.HandleActivity(player, "Dominion tower", 15);
		killedBossesCount += Settings.EVENT_SPOTLIGHT.contains("minigame") ? 2 : 1;
		dominionFactor += factor;
		totalScore += factor;
		player.sendMessage("You now have "+player.getDominionTower().killedBossesCount+" boss kills.");
	//	if (.)
		if (killedBossesCount == 100) {
			World.sendWorldMessage("<col=339966>"+player.getDisplayName()+" has just completed the kill 100 dominion bosses completionist requirement!", false);
		}
