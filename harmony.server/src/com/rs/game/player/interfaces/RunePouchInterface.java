 ...
 * @author Connor
 */
public class RunePouchInterface extends RSInterface {
	...
	// Interface text
	private static final String title = "Rune Pouch", bank = "Bank All", take = "Take All", abandon = "Abandon All";

	// Constructor
	public RunePouchInterface(Player player) {
		this.player = player;
	}
	...
	public boolean onClick(int button, int packet, int itemId, int slotId) {
		Item item = player.getRunePouch().getContainer().get(slotId);
		...
		case 8: // Bank all
			player.getRunePouch().depositChestToBank();
			refreshContainer();
			break;
		...
		case 10: // Take all
			for (Item i : player.getRunePouch().getContainer().getItemsCopy()) {
				if (Objects.isNull(i))
					continue;
				if (player.getInventory().addItem(i)) {
					player.getRunePouch().getContainer().remove(i);
				} else {
					player.getRunePouch().getContainer().remove(new Item(i.getId(), freeSlots));
				}
			}
			refreshContainer();
			break;
	...
	private void refreshContainer() {
		...
		player.getPackets().sendItems(containerKey, player.getRunePouch().getContainer());
	}
