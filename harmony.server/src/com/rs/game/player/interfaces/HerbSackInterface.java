 ...
 * @author Connor
 */
public class HerbSackInterface extends RSInterface {
	...
	// Interface text
	private static final String title = "Herb Sack", bank = "Bank All", take = "Take All", abandon = "Abandon All";

	// Constructor
	public HerbSackInterface(Player player) {
		this.player = player;
	}
	...
	public boolean onClick(int button, int packet, int itemId, int slotId) {
		...
		case 8: // Bank all
			player.getHerbSack().depositChestToBank();
			refreshContainer();
			break;
		...
		case 10: // Take all
			for (Item i : player.getHerbSack().getContainer().getItemsCopy()) {
				if (Objects.isNull(i))
					continue;
				if (player.getInventory().addItem(i)) {
					player.getHerbSack().getContainer().remove(i);
				} else {
					player.getHerbSack().getContainer().remove(new Item(i.getId(), freeSlots));
				}
			}
			refreshContainer();
			break;
	...
	private void refreshContainer() {
		...
		player.getHerbSack().getContainer().shift();
		player.getPackets().sendItems(containerKey, player.getHerbSack().getContainer());
	}
