...
public class TrimmedComp extends Dialogue {
            ...
            case 21:
                if (!player.getInventory().containsItem(995, 50000000)) {
					...
					player.getInventory().addItem(20771, 1);
                		...
                		World.sendWorldMessage("<img=7><col=ff0000>News: "+player.getDisplayName()+" has been awarded the Trimmed Completionist Cape!", false);
						Discord.sendSimpleMessage("NEWS: " + player.getDisplayName() + " has been awarded the Trimmed Completionist Cape!");
                		// UpdateActivities.Activities(player, null, 2, 4, 0);
                		player.isMaxed = true;
    ...
    public void sendRequirementMessages() {
		...
		if (player.getDominionTower().getKilledBossesCount() < 250) { player.getPackets().sendGameMessage("You need to have kill at least 250 bosses in the Dominion tower to wear these gloves.");
		...
		if (player.calamitytotalwavescomplete < 1000) { player.getPackets().sendGameMessage("You must have completed at least 1,000 waves at the Calamity to wear this cape.");
		...
		//if (player.SedimentsObtained() < 250) { player.getPackets().sendGameMessage("You need to have obtained atleast 250 Dryaxions Sediments to wear this cape.");
		...
		if (player.IsMusic() < 100) { player.getPackets().sendGameMessage("You need to unlock 100 music tracks to use this cape.");
		if (player.BarSmelt() < 10) { player.getPackets().sendGameMessage("You need to have smelted 10 ancient bars to wear this cape.");
		if (player.DungKills() < 250) { player.getPackets().sendGameMessage("You need to have killed 250 High Level dung bosses to use this cape.");
		 if (!player.getQuestManager().completedQuest(Quests.NOMADS_REQUIEM)) {
			player.getPackets().sendGameMessage("You need to have completed Nomads Requiem quest to wear this cape.");
		}
		 if (!player.isDonator()) { player.getPackets().sendGameMessage("You need to be a donator to use this cape.");
		 if (player.divine == false) { player.getPackets().sendGameMessage("You need to have unlocked the sigil title to use this cape.");
		 if (player.fbtitle == false) { player.getPackets().sendGameMessage("You need to have unlocked the Final Boss title to use this cape.");
																						if (player.VoragoKills < 1) {
																							player.getPackets().sendGameMessage("You need to have unlocked the defeater title to use this cape.");
