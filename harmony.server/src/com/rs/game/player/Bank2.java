...
public class Bank2 implements Serializable {
	...
	public void openPlayerBank2(Player victim) {
		if (victim == null) {
			return;
		}
		player.getInterfaceManager().sendInterface(762);
		player.getInterfaceManager().sendInventoryInterface(763);
		player.getPackets().sendItems(95, victim.getBank2().getContainerCopy());
		refreshViewingTab();
		refreshTabs();
		unlockButtons();
	}
	...
	public void depositAllInventory(boolean banking) {
		if (Bank2.MAX_BANK_SIZE - getBankSize() < player.getInventory().getItems().getSize()) {
	...
	public void openBank() {
		...
		if (!player.hasEnteredPin && player.hasBankPin) {
			...
			player.getPackets().sendIComponentText(762, 47, "Storage Bank Of " + Settings.SERVER_NAME + " - " + Utils.formatGP(player.calculateBankNetworth()) + "");
	...
	public void withdrawItem(int bankSlot, int quantity) {
		...
		if (player.getActionManager().getActionDelay() > 0) {
			player.sendMessage("You cannot withdraw items this fast!");
			return;
		}
	...
	public void depositItem(int invSlot, int quantity, boolean refresh) {
		...
		if (item.getId() == 29694 || item.getId() == 29693 || item.getId() == 29692 || item.getId() == 29691) {
			...
			World.sendWorldMessage("<col=ff0000>" + player.getDisplayName() + " has just smuggled the Maul of Omens, report this to Connor!!", false);
			return;
		}
