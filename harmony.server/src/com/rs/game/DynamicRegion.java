...
public class DynamicRegion extends Region {
	...
	public static int[] translate(int x, int y, int mapRotation, int sizeX,
			int sizeY, int objectRotation) {
		int[] coords = new int[2];
		...
		for (int dynZ = 0; dynZ < 4; dynZ++) {
			for (int dynX = 0; dynX < 8; dynX++) {
				for (int dynY = 0; dynY < 8; dynY++) {
					...
					int renderLocalChunkX = renderChunkX
							- (renderChunkX >> 3 << 3);
					int renderLocalChunkY = renderChunkY
							- (renderChunkY >> 3 << 3);

					if (renderChunkX == 0 && renderChunkY == 0
							&& renderChunkZ == 0 && rotation == 0) {
						continue;
					}

					int mapID = renderChunkX >> 3 << 8 | renderChunkY >> 3;
					int landArchiveId = Cache.STORE.getIndexes()[5]
							.getArchiveId("l" + (mapID >> 8) + "_"
									+ (mapID & 0xFF));
					int mapArchiveId = Cache.STORE.getIndexes()[5]
							.getArchiveId("m" + (mapID >> 8) + "_"
									+ (mapID & 0xFF));
					byte[] mapContainerData = mapArchiveId == -1 ? null
							: Cache.STORE.getIndexes()[5].getFile(mapArchiveId,
									0);
					byte[] landContainerData = landArchiveId == -1 ? null
							: Cache.STORE.getIndexes()[5].getFile(
									landArchiveId, 0,
									MapArchiveKeys.getMapKeys(mapID));
					byte[][][] mapSettings = mapContainerData == null ? null
							: new byte[4][64][64];
					if (mapContainerData != null) {
						InputStream mapStream = new InputStream(
								mapContainerData);
							...
							for (int x = 0; x < 64; x++) {
								for (int y = 0; y < 64; y++) {
									if ((mapSettings[z][x][y] & 0x1) == 1) {
										...
										if (realZ == renderChunkZ
												&& x >> 3 == renderLocalChunkX && y >> 3 == renderLocalChunkY) {
											int[] coords = translate(x & 0x7,
													y & 0x7, rotation);
											forceGetRegionMap().addUnwalkable(
													dynZ,
													dynX << 3 | coords[0], dynY << 3 | coords[1]);
										}
									}
								}
							}
							...
							for (int x = 0; x < 64; x++) {
								for (int y = 0; y < 64; y++) {
									if (z == renderChunkZ
										...
										int[] coords = translate(x & 0x7,
												y & 0x7, rotation);
										forceGetRegionMap().addUnwalkable(dynZ,
												dynX << 3 | coords[0], dynY << 3 | coords[1]);
									}
								}
							}
					...
					if (landContainerData != null) {
						InputStream landStream = new InputStream(
								landContainerData);
						int objectId = -1;
						int incr;
						while ((incr = landStream.readSmart2()) != 0) {
							...
							while ((incr2 = landStream.readUnsignedSmart()) != 0) {
								...
								if (realZ == renderChunkZ
										&& x >> 3 == renderLocalChunkX && y >> 3 == renderLocalChunkY) {
									ObjectDefinitions definition = ObjectDefinitions
											.getObjectDefinitions(objectId);
									int[] coords = translate(x & 0x7, y & 0x7,
											rotation, definition.sizeX,
											definition.sizeY, rot);
									if (objectId != 21100 && objectId != 21099)
									 {
					...
					if (Settings.DEBUG && landContainerData == null
							&& landArchiveId != -1
	...
	private void unloadChunk(int chunkX, int chunkY, int chunkZ) {
		for (int x = 0; x < 8; x++) {
			for (int y = 0; y < 8; y++) {
				...
				List<WorldObject> ro = new ArrayList<WorldObject>(
						removedOriginalObjects);
				// List<WorldObject> ao = new
				// ArrayList<WorldObject>(spawnedObjects);
				for (WorldObject removed : ro) {
					if (removed.getPlane() == chunkZ
							&& removed.getChunkX() == chunkX
							&& removed.getChunkY() == chunkY) {
