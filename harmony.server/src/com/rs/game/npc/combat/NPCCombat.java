...
public final class NPCCombat {
	...
	private int combatAttack() {
		...
		int maxDistance = attackStyle == NPCCombatDefinitions.MELEE && npc.getId() != 16388 || attackStyle == NPCCombatDefinitions.SPECIAL2 ? 0
				: 7;
	...
	protected void doDefenceEmote(Entity target) {
		 ...
		 * return;
		 */
		if (target.isCantDoDefenceEmote())// Vorago
			return;
		...
		if (!npc.isCantFollowUnderCombat()) {
			...
			/*} else if (npc instanceof Vorago) {
				...
				if ((!npc.clipedProjectile(target, maxDistance == 0)) || distanceX > size + maxDistance
						...
						|| distanceY < -1 - maxDistance) {
					if (!npc.addWalkStepsInteract(target.getX(), target.getY(), 2, size, true)) {
