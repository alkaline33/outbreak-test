package com.rs.game.npc.combat.impl.blink;
...
public class BlinkCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (Utils.random(15) <= 5) // playing his random sounds
			attackSounds(npc);
		if (!specialRapidMagic && npc.getHitpoints() <= 5000 && Utils.random(15) == 2 && !sprayAttack
				|| !specialRapidMagic && npc.getHitpoints() <= 25000
				&& npc.getHitpoints() <= 5000 && !sprayAttack) {
			npc.setNextForceTalk(new ForceTalk(
					(npc.getHitpoints() <= 5000 ? "RIP THEM APART!"
							: "RIP THEM APART!")));
			World.spawnNPC(12865, new WorldTile(2139, 5537, 3), -1, true, true);
			World.spawnNPC(12866, new WorldTile(2146, 5533, 3), -1, true, true);
			World.spawnNPC(12867, new WorldTile(2143, 5539, 3), -1, true, true);
		...
		if (attackStyle == 1 && !specialRapidMagic && !sprayAttack) { // range
			...
			if (Utils.random(3) == 0)
				npc.setNextForceTalk(new ForceTalk("I AM NOT WEAK!!"));
			npc.setNextAnimation(new Animation(14956));
			npc.setNextGraphics(new Graphics(2854));
			for (Entity t : npc.getPossibleTargets()) {
				if (!t.withinDistance(npc, 11))
