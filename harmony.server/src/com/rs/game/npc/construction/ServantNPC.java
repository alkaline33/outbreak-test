...
public class ServantNPC extends NPC {
	...
	/*public void makeFood(final Builds[] builds) {
		...
		final RoomReference kitchen = house.roo(Room.KITCHEN), diningRoom = house.getRoom(Room.DINING_ROOM);
		if (kitchen == null) {
	...
	public void requestType(int item, int quantity, final byte type) {
		...
		//	owner.getDialogueManager().startDialogue("SimpleMessage", "Your servant cannot make it to the bank before the system update is complete.");
		//	return;
		//}
		...
		if (!bank.containsItem(defs.isNoted() ? defs.getCertId() : item) && type == 0) {
		...
		if (type == 1 || type == 2) {
			int amountOwned = owner.getInventory().getNumerOf(item);
		...
		if (plank != null && type == 1) {
			...
			if (owner.getInventory().getNumerOf(995) < cost) {
		...
		if (type == 1 || type == 2) {
			if (type == 2) {
				...
				owner.getInventory().removeItemMoneyPouch(new Item(995, (int) (quantity * (plank.getCost() * 0.7))));
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				setCantInteract(false);
				if (!owner.isRunning() || !house.getPlayers().contains(owner)) {
					if (type == 1 || type == 2) {
				...
				house.incrementPaymentStage();
				if (type == 0) {
					bank.withdrawItem1(bank.getItemSlot(finalItem), completeQuantity);
					owner.getInventory().addItem(finalItem, completeQuantity);
				} else if (type == 1) {
					owner.getInventory().addItem(plank.getId(), completeQuantity);
				} else if (type == 2) {
					owner.getInventory().addItem(finalItem, completeQuantity);
				} else {
					for (int i = 0; i < completeQuantity; i++) {
						//bank.depositItem(owner.getInventory().getItems().getThisItemSlot1(finalItem), completeQuantity, false);
						owner.getDialogueManager().startDialogue("SimpleNPCMessage", getId(), type == 3 ? "Sorry sir, but Harmony said this function is currently not being used." : "Sorry sir, but Harmony said this function is currently not being used.");
					}
				}
				//owner.getPackets().sendGameMessage("If you removed any items they would not has been deposited.", true);
