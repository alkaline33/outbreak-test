...
public class AscensionDungeon {
	...
	public enum AscDoors {
		Primus(new WorldTile(1023, 632, 1), new WorldTile(1025, 632, 1)),
		Secundus(new WorldTile(1106, 673, 1), new WorldTile(1106, 671, 1)),
		Tertius(new WorldTile(1099, 663, 1), new WorldTile(1099, 665, 1)),
		Quartus(new WorldTile(1174, 633, 1), new WorldTile(1176, 633, 1)),
		Quintus(new WorldTile(1193, 634, 1), new WorldTile(1191, 634, 1)),
		Sextus(new WorldTile(1184, 620, 1), new WorldTile(1184, 622, 1));
	...
	public static void handleLightning(Legio leg, WorldTile targetPosition, int min, int max) {
		handleLightning(leg, targetPosition, min, max, false);
	}

	public static void handleLightning(Legio leg, WorldTile targetPosition, int min, int max, boolean sextus) {
			...
			public void run() {
				...
				for (Entity player : leg.getPossibleTargets()) {
					if (Utils.getDistance(targetPosition, player) <= 1) {
						player.applyHit(new Hit(leg, Utils.random(min, max), HitLook.MAGIC_DAMAGE));
					}
				}
			}
		}, 3);
	...
	public static void spawnLine(WorldTile tile, int rot) {
		...
		if (World.getObjectWithId(tile, LINE) != null)
			object = new WorldObject(CROSS_SECTION, 4, rot, tile.getX(), tile.getY(), tile.getPlane());
		else
			object = new WorldObject(LINE, 4, rot, tile.getX(), tile.getY(), tile.getPlane());
	...
	public static void generateLine(Entity player, boolean vertical) {
		...
		if (vertical) {
			for (int y = yMin; y < yMax; y++)
				spawnLine(new WorldTile(player.getX(), y, player.getPlane()), rotation);
		} else {
			for (int x = xMin; x < xMax; x++)
				spawnLine(new WorldTile(x, player.getY(), player.getPlane()), rotation);
		}
	}
	...
	public static boolean spawnBoss(Player player, int ordinal) {
		if (FIGHTS_ACTIVE[ordinal] != null) {
			player.sendMessage("There is already a fight going on inside. Join them when it's over.");
			return false;
		}
		if (player.getInventory().containsItem(getKey(ordinal), 1)) {
			...
			player.sendMessage("You lack the required key to fight this boss.");
			return false;
		}
	}
	...
	public static void exit(Player player, int ordinal, boolean logout, boolean isDeath) {
		if (FIGHTS_ACTIVE[ordinal] != null && FIGHTS_ACTIVE[ordinal].getOwner().equals(player.getUsername())) {
		...
		if(!isDeath) {
			if (logout)
				...
				player.setNextWorldTile(new WorldTile(AscDoors.values()[ordinal].exit.getX(), AscDoors.values()[ordinal].exit.getY(), AscDoors.values()[ordinal].exit.getPlane()));
		}
	}
