...
import com.rs.game.player.content.grandExchange.GrandExchangeConfigurations.Progress;
...
public class WorldPacketsEncoder extends Encoder {
	...
	public void sendIComponentModel(int interfaceId, int componentId,
	 ...
	 public void sendMoveIComponent(int interfaceId, int componentId, int x, int y) {
		  OutputStream stream = new OutputStream(9);
	...
	public void sendNPCOnWidget(int npcId, int inter, int button, boolean fullNPC) {
        OutputStream stream = new OutputStream();
        stream.writePacketVarShort(player, 160);
        stream.writeByte(fullNPC ? 1 : 0);
        stream.endPacketVarShort();
        session.write(stream);
        stream = new OutputStream(9);
        stream.writePacket(player, 31);
        stream.writeInt(npcId);
        stream.writeInt(inter << 16 | button);
        session.write(stream);
    }
	...
	public void sendGrandExchangeBar(Player player, int slot, int progress,
		...
		packet.writePacket(player, 53);
		packet.writeByte(slot);
		packet.writeByte(progress);
		packet.writeShort(item);
		packet.writeInt(price);
		packet.writeInt(amountOffered);
		packet.writeInt(amountSold);
		packet.writeInt(price * amountSold);
		session.write(packet);
	}

	public void sendGrandExchangeBar(Player player, int slot, int item,
			Object progress, int price, int amountSold, int amountOffered) {
	...
	public void setZoomDistance(int interfaceId, int cId, int zoomDis) {
		OutputStream stream = new OutputStream();
		stream.writePacketVarShort(player, 162);
		stream.writeInt(interfaceId);
		stream.writeInt(cId);
		stream.writeInt(zoomDis);
		stream.endPacketVarShort();
		session.write(stream);
	}
	...
	//
	// public void sendItems(int key, ItemsContainer<Item> items, String boss) {
	...
	public void sendTradeRequestMessage(Player p) {
		if (World.TheCalamityWaiting(player)) {
			player.sendMessage("Trading in this area is not permitted.");
			return;
		}
		sendMessage(100, "wishes to trade with you.", p);
	}
	
	public void sendDiceRequestMessage(Player p) {
		if (World.TheCalamityWaiting(player)) {
			player.sendMessage("Dicing in this area is not permitted.");
			return;
		}
		sendMessage(100, "wishes to dice with you.", p);
	}
	...
	public void sendIComponentPosition(int interfaceId, int componentId, int x, int y) {

		OutputStream stream = new OutputStream(8);
		stream.writePacket(player, 104);
		stream.writeShortLE(y);
		stream.writeShortLE(x);
		stream.writeIntV2(interfaceId << 16 | componentId);
		session.write(stream);
	}
