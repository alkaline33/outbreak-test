...
public class ItemExamines {
	...
	public static final String getExamine(int itemId) {//
		return getExamine(new Item(itemId, 1));
	}

	public static final String getShopExamine(Item item) {
	...
	public static final String getExamine(Item item) {
		...
		if (item.getDefinitions().isNoted()) {
			if (item.getDefinitions().getValue() * item.getAmount() < 0) {
				return "Swamp this note at any bank for the equivalent item. Price Checker: <col=ff0000>LOTS";
			} else if (item.getDefinitions().getValue() * item.getAmount() <= 2147483647) {
			return "Swamp this note at any bank for the equivalent item. Price Checker: <col=ff0000>"+Utils.format(item.getDefinitions().getValue() * item.getAmount())+"";
			}
		}
			String examine = itemExamines.get(item.getId());
		if (examine != null) {
			return examine + " Price Checker: <col=ff0000>" + Utils.format(item.getDefinitions().getValue()) + "</col> : Runecoins <col=20EED8>" + Utils.format(RecycleCentreMain.PriceCheck(item.getId())) + "";
		}
		return "It's an " + item.getDefinitions().getName() + ". Price Checker: <col=ff0000>" + Utils.format(item.getDefinitions().getValue() * item.getAmount()) + "</col> : Runecoins <col=20EED8>" + Utils.format(RecycleCentreMain.PriceCheck(item.getId()));
	}
