...
public class RenderAnimDefinitions {
	...
	public static final RenderAnimDefinitions getRenderAnimDefinitions(
			int emoteId) {
	...
	public static void main(String[] args) throws IOException {
		...
		/*for (int i = 0; i < 20000; i++) {
			RenderAnimDefinitions def = getRenderAnimDefinitions(i);
			if (def == null) {
				continue;
			}
			if (def.anInt963 == animId || def.anInt972 == animId) {
				System.out.println("Render anim: " + i);
				break;
			} else if (def.anIntArray967 != null) {
				for (int anim : def.anIntArray967) {
					if (anim == animId) {
						System.out.println("Render anim: " + i);
						break;
					}
