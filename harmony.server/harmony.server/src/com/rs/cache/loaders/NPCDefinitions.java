...
public final class NPCDefinitions {
	...
	//idiot man cass lmao i love him but he is an idiot brb man 2secs :)
	...
	public static final NPCDefinitions getNPCDefinitions(int id) {
		...
		if (def == null) {
			...
			if(def.name != null) {
				//System.out.println("Adding npc: " + def.name + " : " + def.npcId);
				if(NPCDrops.getDrops(id) != null) {
					if(NPCDrops.getDrops(id).length != 0) {
						npcDefinitionsByName.put(def.name, id);

					} else {
						System.out.println("PFF nulled drops for: " + id + " : " + NPCDrops.getDrops(id).length);
					}
	...
	public void setTempRenderEmote(int id) {//gonna play with render emote later
		renderEmote = id;
	}
