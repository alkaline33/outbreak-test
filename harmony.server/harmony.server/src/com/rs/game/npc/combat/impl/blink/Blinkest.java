...
public class Blinkest extends CombatScript { //MELEE MINION
	// blue gfxs - 2854-2855
	// blue ranged - 2853
	...
	public int attack(final NPC npc, final Entity target) {
				...
				public void run() {
					...
					for (Entity t : npc.getPossibleTargets()) {
						if (!t.withinDistance(npc, 10))
							continue;
						delayHit(npc, 1, t, new Hit(npc, Utils.random(50),
								HitLook.MELEE_DAMAGE));
						t.setNextGraphics(new Graphics(2855, 0, 0));
					}
		...
		if (attackStyle == 1 && !specialRapidMagic && !sprayAttack) { // range
			...
			if (Utils.random(3) == 0)
				npc.setNextForceTalk(new ForceTalk("Mageinyaface!"));
			npc.setNextAnimation(new Animation(14956));
			npc.setNextGraphics(new Graphics(2854));
			for (Entity t : npc.getPossibleTargets()) {
				if (!t.withinDistance(npc, 11))
