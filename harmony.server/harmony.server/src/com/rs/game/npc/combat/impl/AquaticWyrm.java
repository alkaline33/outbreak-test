...
public class AquaticWyrm extends CombatScript {

	@Override
	public Object[] getKeys() {
		return new Object[] { 30085 };
	}
	...
	private void sendMagicAttack(NPC npc, int emote) {
		...
		for (final Entity target : targets) {
			if (target == null)
				continue;
			World.sendProjectile(npc, target, 500, 50, 25, 45, 35, 16, 0);
			final int hitAmount = 1 + (Utils.getRandom(1) == 1 ? Utils.getRandom(1) : 0);
			ArrayList<Hit> hits = new ArrayList<Hit>(hitAmount);
			hits.add(new Hit(npc, getRandomMaxHit(npc, 500, NPCCombatDefinitions.MAGE, target), HitLook.MAGIC_DAMAGE));
			if (hitAmount > 1) {
				hits.add(new Hit(npc, Utils.random(50, 250), HitLook.DESEASE_DAMAGE));
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							((Player) target).getPackets().sendGameMessage(
									...
									"It seems that the water suffocated you.");
						}
					}, 1);
				}
			}
	...
	private void sendRangedAttack(NPC npc, int emote) {
		...
		for (final Entity e : npc.getPossibleTargets()) {
			if (e != null && e.withinDistance(npc.getMiddleWorldTile(), 4) && Utils.getRandom(1) == 0) {
				e.setNextGraphics(new Graphics(1026));
				delayHit(npc, 1, e, getRangeHit(npc, getRandomMaxHit(
						npc, 450, NPCCombatDefinitions.RANGE, e)));
