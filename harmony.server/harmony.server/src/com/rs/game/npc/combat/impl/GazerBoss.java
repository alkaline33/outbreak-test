...
public class GazerBoss extends CombatScript {

	@Override
	public Object[] getKeys() {
		return new Object[] { 9752 };
	}
	...
	public int attack(final NPC npc, final Entity target) { //melee
		...
		if (Utils.getRandom(3) == 0) { //Range hit
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					npc.setNextAnimation(new Animation(13420));
					target.setNextGraphics(new Graphics(2787));
					delayHit(
							npc,
							1,
							target,
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						for (int[] tile : tiles.values()) {
							npc.setNextGraphics(new Graphics(2786));
							World.sendGraphics(null, new Graphics(2788),
									new WorldTile(tile[0], tile[1], 0));
							for (Entity t : possibleTargets)
							//	if (t.getX() == tile[0] && t.getY() == tile[1])
									t.applyHit(new Hit(npc,
											Utils.getRandom(50) + 25,
