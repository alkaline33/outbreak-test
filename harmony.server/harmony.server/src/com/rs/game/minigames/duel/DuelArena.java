...
public class DuelArena extends Controler {
	...
	public void start() {
		...
		target.getInterfaceManager().closeChatBoxInterface();
		player.getInterfaceManager().closeChatBoxInterface();
		if (player.getEquipment().hasAura()) {
			player.sendMessage(Colors.red + "Aura's are not allowed in the duel arena!");
			return;
		}
		if (target.getEquipment().hasAura()) {
			player.sendMessage(Colors.red + "Your target is using an aura!");
			return;
		}
		if (!DuelControler.isAtDuelArena(target)) {
			player.sendMessage(Colors.red + "Your target has left the duel arena!");
			return;
		}
		if (!DuelControler.isAtDuelArena(player)) {
			player.sendMessage(Colors.red + "You have left the duel arena!");
			return;
		}
	...
	private void openDuelScreen(Player target, boolean ifFriendly) {
		...
		if (!ifFriendly && player.ironman == true) {
			player.sendMessage("This account cannot use the duel arena to stake items!");
			return;
		}
		if (!ifFriendly && target.ironman == true) {
			player.sendMessage("This account cannot use the duel arena to stake items!");
			return;
		}
		// target.getInterfaceManager().closeInterfaceCustom();
	...
	private void accept(boolean firstStage) {
		...
		if (player.ironman == true || player.isPvpMode()) {
			player.sendMessage("This account cannot use the duel arena!");
			return;
		}
		if (player.getPolDelay() > 0) {
			player.sendMessage("You cannot use the duel arena whilst under a special effect.");
			return;
		}
	...
	public void addItem(int slot, int amount) {
		...
		if (item.getId() != 995 && item.getId() != 5020 && item.getId() != 5022 && item.getId() != 5023) {
			player.sendMessage(Colors.red + "The duel arena is currently only allowing players to stake coins or tickets.");
			return;
		}
	...
	public boolean nextStage() {
		...
		if (player.getInventory().getItems().getUsedSlots() + player.getLastDuelRules().getStake().getUsedSlots() + target.getLastDuelRules().getStake().getUsedSlots() > 28) {
			player.setCloseInterfacesEvent(null);
			player.closeInterfaces();
		...
		if (player.getPet() != null || Pets.hasPet(player)) {
			player.sendMessage("You cannot bring pets to the duel arena!");
			player.setCloseInterfacesEvent(null);
			player.closeInterfaces();
			closeDuelInteraction(true, DuelStage.NO_SPACE);
			return false;
		...
		if (isDueling) {
			for (Item item : player.getLastDuelRules().getStake().getItems()) {
				...
				if (loser.isDead() && !victor.isDead()) {
					...
					player.getInventory().addItem(item); // refunds items
					player.getPackets().sendGameMessage("Your fight ended in a draw! Your stake was refunded.");
				}
				victor.printDuelArena(victor, "won" + item.getAmount() + " x " + item.getName() + ".");
				loser.printDuelArena(loser, "lost" + item.getAmount() + " x " + item.getName() + ".");
				loser.sendMessage(Colors.red + "You have lost: " + item.getAmount() + " x " + item.getName() + ".");
			}
	...
	private void removeEquipment() {
		...
		for (int i = 10; i < 23; i++) {
			if (i == 13) {
				ButtonHandler.sendRemove(target, 13);
			}
			if (i == 14) {
	...
	public boolean processObjectClick1(WorldObject object) {
		...
		player.sendMessage("You cannot forfit from the duel arena.");
		return true;
	}
	...
	public boolean processButtonClick(int interfaceId, int componentId, int slotId, int packetId) {
		synchronized (this) {
			if (target.getControlerManager().getControler() == null) {
				player.sendMessage("Your target wasn't in the duel screen, so we have ended this duel.");
				player.unlock();
				player.getInterfaceManager().closeInterfaceCustom();
				player.getControlerManager().removeControlerWithoutCheck();
			}
			synchronized (target.getControlerManager().getControler()) {
				DuelRules rules = player.getLastDuelRules();
				if (interfaceId == 749 && componentId == 4 && rules.getRule(5) && isDueling) {
				...
				case 631:
					...
					case 59: // fun wep
						// rules.setRules(8);
						player.sendMessage("Fun weapons are disabled. This feature will not activate.");
						return false;
					case 60: // no forfiet
						rules.setRules(7);
						return false;
