...
public class Sawmill {
	...
	public static void handlePlanksConvertButtons(Player player, int componentId, int packetId) {
		if (componentId >= 12 && componentId <= 15) {
			...
			if (packetId == WorldPacketsDecoder.ACTION_BUTTON4_PACKET) {
				...
				player.getPackets().sendInputIntegerScript(false, "Enter amount:");
				return;
			}
			int amount = packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET ? 1 : packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET ? 5 : packetId == WorldPacketsDecoder.ACTION_BUTTON3_PACKET ? 10 : player.getInventory().getNumerOf(type.logId);
	...
	public static void convertPlanks(Player player, Plank type, int amount) {
		int warning = 0;
		int logsAmt = player.getInventory().getNumerOf(type.logId);
		if (amount > logsAmt) {
			amount = logsAmt;
			warning = 1;
		}
		int cost = amount * type.cost;
		int invCoins = player.getInventory().getNumerOf(995);
		if (cost > invCoins) {
