...
public final class RegionBuilder {
    ...
    public static int findEmptyRegionHash(int widthChunks, int heightChunks) {
		...
		synchronized (ALGORITHM_LOCK) {
			...
			skip: 
				for (int regionY = 1; regionY <= MAX_REGION_Y - regionsDistanceY; regionY++) {
					final int regionHash = getRegionId(regionX, regionY); // map hash because skiping to next map up
				    for (int checkRegionX = regionX - 1; checkRegionX <= regionX + regionsDistanceX; checkRegionX++) {
						for (int checkRegionY = regionY - 1; checkRegionY <= regionY + regionsDistanceY; checkRegionY++) {
						    ...
						    if (regionExists(hash)) {
						    	//World.sendWorldMessage("<col=ff0000>["+MAX_REGION_X+"]["+MAX_REGION_Y+"] Region exists! Hash: "+hash+", real x: "+regionX+", real y: "+regionY+", check x: "+checkRegionX+", check y: "+checkRegionY);
						    	continue skip;
						    }
						}
				    }
    ...
    public static void destroyRegion(int regionId) {
    	synchronized (ALGORITHM_LOCK) {//TODO WAS PREVIOUSLY SYCNHED
			Region region = World.getRegions().get(regionId);
			if (region != null) {
			    ...
			    if (npcIndexes != null) {
					for (int npcIndex : npcIndexes) {
					    ...
					    if (npc instanceof Familiar || npc instanceof Pet) {
					    	npc.setLocation(0, 0, 0); // sets 0,0,0 and forces to teleto player and not force load region and loss space
							continue;
					    }
					    npc.finish();
					}
