...
public class BuyGeInterface {
        ...
        public static void sendInterface(Player player) throws IOException {
			...
			if (offer.isAborted() || offer.isFinished() || offer.isProcessing())
                                continue; // skip offers aborted or finished
                        
                        if (gePrice <  ItemDefinitions.getItemDefinitions(ItemId).getValue() / 3)
                        	continue;
                        
                        if (Type1.name().contentEquals("SELL")) 
                        	continue;
                       ...
                       int amountleft = amount - sold;
                        ...
                        player.getPackets().sendIComponentText(275,1,"Harmony Grand Exchange Buying List");
                        player.getPackets().sendIComponentText(275, 10 + (pos), "<col=000066>"+amountleft+ " x " + ItemN + " for " +Utils.format(gePrice)+ " each.");
                        
                         //   player.getPackets().sendIComponentText(275, 13 + (pos), "<col=000066><u>" + owner);
