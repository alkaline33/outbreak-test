...
public final class LevelUp extends Dialogue {
	...
	public void start() {
		...
		if (player.getRights() < 3 && (level == 99 || level == 120) && skill >= 0) {
			sendNews(player, skill, level);
		}
	}
	
	public static void send104m(Player player, int skill) {
		String icon = player.isHCIronman() ? "<img=17>" : player.isIronman() ? "<img=15>" : "";
		World.sendWorldMessage(
				"<img=6><col=ff8c38>News: [" + player.getXpMode() +"] " + icon + player.getDisplayName() + " has achieved virtual level 120 in " + Skills.SKILL_NAME[skill] + "!", false);
		return;
	}
	...
	public static void send300m(Player player, int skill) {
		String icon = player.isHCIronman() ? "<img=17>" : player.isIronman() ? "<img=15>" : "";
		World.sendWorldMessage(
				"<img=6><col=ff8c38>News: [" + player.getXpMode() +"] " + icon + player.getDisplayName() + " has achieved 300m experience in " + Skills.SKILL_NAME[skill] + "!", false);
		return;
	}
	
	public static void send360m(Player player, int skill) {
		String icon = player.isHCIronman() ? "<img=17>" : player.isIronman() ? "<img=15>" : "";
		World.sendWorldMessage(
				"<img=6><col=ff8c38>News: [" + player.getXpMode() +"] " + icon + player.getDisplayName() + " has achieved 360m experience in " + Skills.SKILL_NAME[skill] + "!", false);
		return;
	}
	...
	public static void send200m(Player player, int skill) {
		String icon = player.isHCIronman() ? "<img=17>" : player.isIronman() ? "<img=15>" : "";
		World.sendWorldMessage(
				"<img=6><col=ff8c38>News: [" + player.getXpMode() +"] " + icon + player.getDisplayName() + " has achieved 200m experience in " + Skills.SKILL_NAME[skill] + "!", false);
		return;
	}
	...
	public static void sendNews(Player player, int skill, int level) {
		...
		if(!reachedAll) {
		    World.sendWorldMessage("<img=6><col=ff8c38>News: [" + player.getXpMode() +"] " + icon + player.getDisplayName() + " has achieved "+level+" "+Skills.SKILL_NAME[skill]+".", false);
		    return;
		} else
		World.sendWorldMessage("<img=6><col=ff0000>News: [" + player.getXpMode() +"] " + icon + player.getDisplayName() + " has just achieved at least level 99 in all skills!", false);
		player.sendMessage("Max capes have been added to your bank. ;;maxtitle for your new title.");
		player.getBank().addItem(20767, 10, true);
		player.getBank().addItem(20768, 10, true);
		player.maxed = true;
	}
