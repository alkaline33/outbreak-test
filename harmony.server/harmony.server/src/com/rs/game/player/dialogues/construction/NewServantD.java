...
public class NewServantD extends Dialogue {
	...
	private void sendBeginningOption() {
		if (servant.getServantData().isSawmill()) {
			sendOptionsDialogue("Select an Option", "Doesnt work", "Bring something from the bank", "Take something to the sawmill");
		} else {
			sendOptionsDialogue("Select an Option", "Doesnt work", "Bring something from the bank");
		}
		stage = 9;
	}
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			if (componentId == OPTION_1) {
				...
				if (player.getInventory().getNumerOf(995) < cost) {
