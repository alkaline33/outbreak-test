	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			if (componentId == OPTION_1) {
				...
				if (!(player.getControlerManager().getControler() instanceof ZGDControler))
				player.getControlerManager().startControler("ZGDControler");//prevents a server null, used it on eradicationx 2 days ago lolo
			}
			end();
		}
