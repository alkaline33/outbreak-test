...
public final class Combat {
	...
	public static boolean hasAntiDragProtection(Entity target) {
		...
		for (int bosspet : Settings.PETS_WITH_PERKS) {
			if (p2.getPetManager().getNpcId() == bosspet && p2.getPetPerk().getContainer().contains(new Item(29384))) {
				return true;
			}
		}
		int shieldId = p2.getEquipment().getShieldId();
		return shieldId == 1540 || shieldId == 11283 || shieldId == 11284 || shieldId == 29196 || shieldId == 29187;
	}

	public static boolean hasWyvernProtection(Entity target) {
		if (target instanceof NPC)
			return false;
		Player p2 = (Player) target;
		int shieldId = p2.getEquipment().getShieldId();
		return shieldId == 11283 || shieldId == 2890 || shieldId == 9731 || shieldId == 29196;
	}
			...
			if (shieldId == -1
				...
				if (weaponName != null && !weaponName.equals("null")) {
					...
					if (weaponName.contains("2h sword")
							|| weaponName.contains("anger sword")
