...
public final class Runecrafting {
	...
	public static void ZMIhandler(Player player) {
		int actualLevel = player.getSkills().getLevel(Skills.RUNECRAFTING);
		if (actualLevel < 5) {
			craftEssence(player, 556, 1, 10, false, 11, 2, 22, 3, 34, 4, 44, 5, 55, 6, 66, 7, 77,
					88, 9, 99, 10);
		} else if (actualLevel >= 5 && actualLevel < 9) {
			Runecrafting.craftEssence(player, 555, 5, 12, false, 19, 2, 38, 3, 57, 4, 76, 5, 95, 6);
		} else if (actualLevel >= 9 && actualLevel < 14) {
			Runecrafting.craftEssence(player, 557, 9, 13, false, 26, 2, 52, 3, 78, 4);
		} else if (actualLevel >= 14 && actualLevel < 20) {
			Runecrafting.craftEssence(player, 554, 14, 14, false, 35, 2, 70, 3);
		} else if (actualLevel >= 20 && actualLevel < 27) {
			Runecrafting.craftEssence(player, 559, 20, 15, false, 46, 2, 92, 3);
		} else if (actualLevel >= 27 && actualLevel < 35) {
			Runecrafting.craftEssence(player, 564, 27, 16, true, 59, 2);
		} else if (actualLevel >= 35 && actualLevel < 40) {
			Runecrafting.craftEssence(player, 562, 35, 17, true, 74, 2);
		} else if (actualLevel >= 40 && actualLevel < 45) {
			Runecrafting.craftEssence(player, 9075, 40, 17.4, true, 82, 2);
		} else if (actualLevel >= 45 && actualLevel < 54) {
			Runecrafting.craftEssence(player, 561, 45, 18, true, 91, 2);
		} else if (actualLevel >= 54 && actualLevel < 65) {
			Runecrafting.craftEssence(player, 563, 50, 19, true);
		} else if (actualLevel >= 65 && actualLevel < 77) {
			Runecrafting.craftEssence(player, 560, 65, 20, true);
		} else if (actualLevel >= 77) {
			Runecrafting.craftEssence(player, 565, 77, 21, true);
		}
	}
	...
	public static void craftEssence(Player player, int rune, int level,
		...
		if (runes > 0) {
			if (Utils.random(1, 10) == 1 && player.getEquipment().getCapeId() == 29980) {
				player.sendMessage("Your master runecrafting cape has saved your essence.");
			} else if (Utils.random(1, 10) == 1 && player.getEquipment().getCapeId() == 20771 && player.getSkills().getXp(Skills.RUNECRAFTING) >= 104273167) {
				player.sendMessage("Your master runecrafting cape has saved your essence.");
			} else {
		...
		if (rune == 556 && player.challengeamount > 0 && player.challengeid == 4) {
			DailyChallenges.UpdateRCChallenge(player, runes);
		}
		if (rune == 560 && player.challengeamount > 0 && player.challengeid == 5) {
			DailyChallenges.UpdateRCChallenge(player, runes);
		}
		if (rune == 566 && player.challengeamount > 0 && player.challengeid == 6) {
			DailyChallenges.UpdateRCChallenge(player, runes);
		}
		player.getPackets().sendGameMessage(
