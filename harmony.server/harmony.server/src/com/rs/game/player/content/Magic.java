...
import com.rs.game.world.timers.VengeTimer;
...
public class Magic {
	...
	public static final boolean hasInfiniteRunes(int runeId, int weaponId,
			int shieldId) {
		if (ItemDefinitions.getItemDefinitions(weaponId).getName().contains("of Sliske")) {
			return true;
		}
		if (runeId == AIR_RUNE) {
			// if (
			if (weaponId == 29546 || weaponId == 22347 || weaponId == 29451 || weaponId == 1381 || shieldId == 19889 || weaponId == 21777 || weaponId == 1397) {
				return true;
			}
		} else if (runeId == WATER_RUNE) {
			if (weaponId == 29546 || weaponId == 22347 ||weaponId == 1383 || shieldId == 18346 || shieldId == 19889 || weaponId == 1395) {
				return true;
			}
		} else if (runeId == EARTH_RUNE) {
			if (weaponId == 29546 || weaponId == 22347 ||weaponId == 1385 || weaponId == 1399 || weaponId == 3053 || weaponId == 3055 || shieldId == 19889) {
				return true;
			}
		} else if (runeId == FIRE_RUNE) {
			if (weaponId == 29546 || weaponId == 22347 || weaponId == 29451 || weaponId == 3053 || shieldId == 29286 || weaponId == 1387 || weaponId == 1393 || shieldId == 19889) {
				return true;
			}
		} else if (runeId == MIND_RUNE || runeId == NATURE_RUNE || runeId == CHAOS_RUNE || runeId == DEATH_RUNE
				|| runeId == BLOOD_RUNE|| runeId == SOUL_RUNE|| runeId == ASTRAL_RUNE|| runeId == LAW_RUNE
				|| runeId == STEAM_RUNE|| runeId == MIST_RUNE|| runeId == DUST_RUNE|| runeId == SMOKE_RUNE
				|| runeId == MUD_RUNE|| runeId == LAVA_RUNE|| runeId == ARMADYL_RUNE) {
			if (weaponId == 29546) {
				return true;
			}
		}
	...
	public static final boolean checkCombatSpell(Player player, int spellId,
		...
		case 192:
			...
			case 87:
				...
				break;
			case 89:
				if (!checkSpellRequirements(player, 90, delete, EARTH_RUNE, 10,
						AIR_RUNE, 7, DEATH_RUNE, 1, BLOOD_RUNE, 1)) {
					return false;
				}
				break;
			...
			case 68: // Flame of Zammy
				...
				break;
			case 91:
				if (!checkSpellRequirements(player, 95, delete, FIRE_RUNE, 10,
						AIR_RUNE, 7, DEATH_RUNE, 1, BLOOD_RUNE, 1)) {
					return false;
				}
				break;
	...
	public static final boolean checkRunes(Player player, boolean delete,
		...
		while (runesCount < runes.length) {
			...
			if (player.getInventory().contains(runepouch)) {
				if (player.getRunePouch().getContainer().contains(new Item(runeId))) {
					continue;
				}
			}
			if (player.getInventory().contains(runepouchupgraded)) {
				if (player.getRunePouchUpgraded().getContainer().contains(new Item(runeId))) {
					continue;
				}
			}
		...
		if (delete) {
			...
			while (runesCount < runes.length) {
				...
				if (player.savespellrunes && Utils.random(5) == 0) {
				} else {
				if (player.getInventory().contains(runepouch) && player.getRunePouch().getContainer().contains(new Item(runeId))) {
					player.getRunePouch().getContainer().remove(new Item(runeId, ammount));
				} else if (player.getInventory().contains(runepouchupgraded) && player.getRunePouchUpgraded().getContainer().contains(new Item(runeId))) {
					player.getRunePouchUpgraded().getContainer().remove(new Item(runeId, ammount));
				} else {
	...
	public static final void sendSkyJumpTeleport(Player player, int level,
			double xp, WorldTile tile, int... runes) {

		sendTeleportSpell(player, -1, -1, -1 , -1, level, xp, tile, 22, true,
				MAGIC_TELEPORT, runes);
	}
	
	public static void BroomTele(final Player player,
			int i, int j, final WorldTile tile) {
		if (!player.getControlerManager().processObjectTeleport(tile)) {
			return;
		}
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					player.setNextAnimation(new Animation(10538));
					player.setNextGraphics(new Graphics(974));
					player.setNextGraphics(new Graphics(1896));
				} else if (loop == 2) {
					player.setNextWorldTile(tile);
				player.setNextAnimation(new Animation(10537));
				player.setNextGraphics(new Graphics(1897));
				} else if (loop == 4) {
	...
	public static void SkyJumpTeleport(final Player player,
			int i, int j, final WorldTile tile) {
	...
	public static final void sendGnomeTeleportSpell(Player player, int level,
			double xp, WorldTile tile, int... runes) {
		if (World.TheCalamity(player)
				|| player.getControlerManager().getControler() instanceof CalamityPlaying) {
			player.sendMessage("You cannot teleport from here.");
			return;
		}
		sendTeleportSpell(player, 17191, -1, 3254 , -1, level, xp, tile, 22, true,
				MAGIC_TELEPORT, runes);
	}
	...
	public static final void sendDrygonTeleportSpell(Player player, int level,
			double xp, WorldTile tile, int... runes) {
		if (World.TheCalamity(player)
				|| player.getControlerManager().getControler() instanceof CalamityPlaying) {
			player.sendMessage("You cannot teleport from here.");
		...
		sendTeleportSpell(player, 14300, -1, 122 , -1, level, xp, tile, 5, false,
				MAGIC_TELEPORT, runes);
	}
	...
	public static final void sendNormalTeleportSpell(Player player, int level,
		...
		sendTeleportSpell(player, 8939, 8941, 1576, 1577, level, xp, tile, 3,
				false, MAGIC_TELEPORT, runes);
	}
	
	public static final void sendUnicornTeleportSpell(Player player, int level,
			double xp, WorldTile tile, int... runes) {
		if (World.TheCalamity(player)
				|| player.getControlerManager().getControler() instanceof CalamityPlaying) {
			player.sendMessage("You cannot teleport from here.");
		...
		sendTeleportSpell(player, 17106, -1, 3223 , -1, level, xp, tile, 18, true,
				MAGIC_TELEPORT, runes);
	}
	
	public static final void sendStalkTeleportSpell(Player player, int level,
			double xp, WorldTile tile, int... runes) {
		if (World.TheCalamity(player)
				|| player.getControlerManager().getControler() instanceof CalamityPlaying) {
			player.sendMessage("You cannot teleport from here.");
		...
		sendTeleportSpell(player, 3094, -1, 1000 , -1, level, xp, tile, 3, true,
				MAGIC_TELEPORT, runes);
	}
	
	public static final void sendCloudTeleportSpell(Player player, int level,
			double xp, WorldTile tile, int... runes) {
		if (World.TheCalamity(player)
				|| player.getControlerManager().getControler() instanceof CalamityPlaying) {
			player.sendMessage("You cannot teleport from here.");
		...
		sendTeleportSpell(player, 17106, -1, 3310 , -1, level, xp, tile, 18, true,
				MAGIC_TELEPORT, runes);
	}
	...
	public static final void sendDemonTeleportSpell(Player player, int level,
		...
		sendTeleportSpell(player, 17108, -1, 3225 , 3224, level, xp, tile, 18, true,
				MAGIC_TELEPORT, runes);
	}
	public static final void sendWhirlpoolTeleportSpell(Player player, int level,
			double xp, WorldTile tile, int... runes) {
		if (World.TheCalamity(player)
				|| player.getControlerManager().getControler() instanceof CalamityPlaying) {
			player.sendMessage("You cannot teleport from here.");
		...
		sendTeleportSpell(player, 6723, -1, -1 , -1, level, xp, tile, 6, true,
				MAGIC_TELEPORT, runes);
	}
	
	public static final void sendLuthTeleportSpell(Player player, int level,
			double xp, WorldTile tile, int... runes) {
		if (World.TheCalamity(player)
				|| player.getControlerManager().getControler() instanceof CalamityPlaying) {
			player.sendMessage("You cannot teleport from here.");
		...
		sendTeleportSpell(player, 17101, -1, 3222 , 3221, level, xp, tile, 18, true,
				MAGIC_TELEPORT, runes);
	}
	...
	public static final boolean sendTeleportSpell(final Player player,
		...
		if (World.TempleofLight(player)) {
			player.sendMessage(Colors.cyan + "You cannot teleport from here!");
			return false;
		}
		if (player.getControlerManager().getControler() instanceof HouseControler) {
			player.sendMessage("Please leave the house via the portal first.");
			return false;
		}
