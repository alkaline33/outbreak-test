...
public final class DungeonUtils {
	...
	public static int getGuardianCreature(int type) {
		return DungeonConstants.GUARDIAN_CREATURES[type][Utils.random(DungeonConstants.GUARDIAN_CREATURES[type].length)];
	}
	...
	public static Room getStartRoom(int type, int complexity, RoomReference reference, Room[][] map) {
		Room[] possibleRooms = selectPossibleRooms(reference, map, DungeonConstants.START_ROOMS[type], true, false, complexity);
	...
	public static Room getPuzzleRoom(int type, int complexity, RoomReference reference, Room[][] map) {
		Room[] possibleRooms = selectPossibleRooms(reference, map, DungeonConstants.PUZZLE_ROOMS[type], true, true, complexity);
		if(possibleRooms.length == 0)
			return null;
		return possibleRooms[Utils.random(possibleRooms.length)];
	}
	...
	public static Room getNormalRoom(int type, int complexity, RoomReference reference, Room[][] map) {
		Room[] possibleRooms = selectPossibleRooms(reference, map, DungeonConstants.NORMAL_ROOMS[type], true, true, complexity);
		if(possibleRooms.length == 0)
			return null;
		return possibleRooms[Utils.random(possibleRooms.length)];
	}
	...
	public static Room[] selectPossibleRooms(RoomReference reference, Room[][] map, HandledRoom[] handledRooms, boolean checkBounds, boolean checkMap, int complexity) {
		...
		for(HandledRoom handledRoom : handledRooms) {
			...
			for(int rotation = 0; rotation < DungeonConstants.ROTATIONS_COUNT; rotation++) {
				...
				if(checkMap) {	
					...
					checkRoom = reference.getY() == map[0].length-1 ? null : map[reference.getX()][reference.getY()+1];
					if(checkRoom != null) {
