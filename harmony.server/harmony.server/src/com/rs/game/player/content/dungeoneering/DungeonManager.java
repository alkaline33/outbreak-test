...
public class DungeonManager {
	...
	public WorldTile getRoomCenterTile(RoomReference reference) {
		return new WorldTile(((boundChuncks[0] << 3) + reference.getX() * 16) + 8,
				((boundChuncks[1] << 3) + reference.getY() * 16)+ 8
				, 0);
	}
	...
	public void linkPartyToDungeon() {
		for(DungeonPartyPlayer dplayer : party.getTeam()) {
			...
			player.getPackets().sendGameMessage("Floor "+party.getFloor()+" Complexity "+party.getComplexity()+" (Full)");
			player.getPackets().sendGameMessage("Dungeon Size: "+ "Small");
			player.getPackets().sendGameMessage("Party Size:Dificulty "+party.getTeam().size()+":"+party.getTeam().size());
			player.getPackets().sendGameMessage("");
		}
	}
	...
	public void spawnNPC(RoomReference reference, int id, int x, int y, boolean check, boolean guardian) {
		...
		WorldTile tile = new WorldTile(((boundChuncks[0] << 3) + reference.getX() * 16) + x,
				((boundChuncks[1] << 3) + reference.getY() * 16)+ y
				, 0);
		...
		CoresManager.slowExecutor.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					boundChuncks = RegionBuilder.findEmptyChunkBound(
							dungeon.getMapWidth() * 2,
							dungeon.getMapHeight() * 2);
					 //reserves all map area
					RegionBuilder.cutMap(boundChuncks[0], boundChuncks[1], dungeon.getMapWidth()*2, dungeon.getMapHeight()*2, 0);
					//loads start room
