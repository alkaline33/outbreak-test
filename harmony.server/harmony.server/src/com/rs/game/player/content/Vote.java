...
public class Vote {
	...
	public static void checkVote(final Player player, final String auth) {
		...
		CoresManager.slowExecutor.execute(new Runnable() {
			...
			public void run() {
				try {
					URL url = new URL("http://www.matrixftw.com/includes/modules/vote/check_vote.php?authentication=apacheownsyou&auth="+auth.toLowerCase());
					...
					if(line.equals("SUCCESS_AUTH_"+auth)) {
						...
						player.getDialogueManager().startDialogue("SimpleMessage", "Thank you for voting, you've receivedthe availability to wear", "prod items for 24 hours.");
						...
						player.getDialogueManager().startDialogue("SimpleMessage", "Thank you for voting, you've receivedthe availability to wear", "prod items for 24 hours and a weekly gift on your bank.");
						player.getPackets().sendGameMessage("You have received: " +
								ItemDefinitions.getItemDefinitions(Integer.parseInt(weekly_gift[3])).getName()+", in your bank.");
					} else 
