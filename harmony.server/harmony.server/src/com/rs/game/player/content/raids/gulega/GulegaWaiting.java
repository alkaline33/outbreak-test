...
public final class GulegaWaiting {
	...
	public static final WorldTile LOBBY = new WorldTile(Settings.RESPAWN_PLAYER_LOCATION),
			TRIO2_WAITING = new WorldTile(2773, 2794, 0),
			TRIO1_WAITING = new WorldTile(2773, 2794, 0),
			TRIO1_BASE = new WorldTile(2389, 10279, 1),
			TRIO2_BASE = new WorldTile(2389, 10279, 1);
	
	/**
	 * Author - Mr_Joopz - 2017
	 */
	...
	public static void joinPortal(Player player, int team) {
		String raidedMacs = player.getMacAddress();
		...
		if (player.getInventory().getFreeSlots() < 28 || player.getEquipment().wearingArmour()) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You cannot take items into this raid!");
			player.sendMessage("You're not permitted to take items into this raid.");
			return;
		}
		...
		player.getControlerManager().startControler("GulegaWaitingC", team);
	...
	//	System.out.println("4");
		for (NPC n : World.getNPCs()) {
			if (n == null || n.getId() < 30087 || n.getId() > 30087)
				continue;
			n.sendDeath(n);
		}
		System.out.println("3");
		Settings.GULEGAENTER = true;
		playingGame.cancel();
		playingGame = null;
		refreshAllPlayersTime();
		leavePlayersSafely();
		World.sendWorldMessage("<col=ff0000> The Gulega raid now has space for a new game!", false);
	}
	...
	public static void refreshTimeLeft(Player player) {
		player.getPackets().sendConfig(380, playingGame == null ? 0 : playingGame.minutesLeft
				- (player.getControlerManager()
						.getControler() instanceof GulegaPlaying ? 10
								: 0));
	}
	...
	public static void joinPlayingGame(Player player, int team) {
		...
		player.raidsfee++;
		player.getControlerManager().startControler("GulegaPlaying", team);
		...
		if (team == 1) {
			...
			if(npcs == null || npcs.isEmpty() || npcs.size() == 8)  {
				//System.out.println("2");
				World.spawnNPC(30087, new WorldTile(2385, 10252, 1), -1, true);
			}
		}
		Settings.GULEGAENTER = false;
	}
