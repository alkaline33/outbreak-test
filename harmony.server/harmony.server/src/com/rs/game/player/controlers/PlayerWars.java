...
import com.rs.game.npc.others.PlayerWarsFinal;
...
public class PlayerWars extends Controler {

	public static final WorldTile OUTSIDE = new WorldTile(Settings.RESPAWN_PLAYER_LOCATION);

	private static final int THHAAR_MEJ_JAL = 817;
	...
	public static void enterPlayerWars(Player player) {
		if(player.getFamiliar() != null || player.getPet() != null || Summoning.hasPouch(player) || Pets.hasPet(player)) {
			player.getDialogueManager().startDialogue("SimpleNPCMessage", THHAAR_MEJ_JAL, "No pets are allowed inside here.");
			return;
		}
		player.hptracker = false;
		player.getControlerManager().startControler("PWControler", 1); //start at wave 1
	}
	...
	public void loadCave(final boolean login) {
		...
		CoresManager.slowExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				//552 640
				RegionBuilder.copyAllPlanesMap(401, 427, boundChuncks[0], boundChuncks[1], 2);
				//selects a music
				...
				WorldTasksManager.schedule(new WorldTask()  {
					...
					public void run() {
						...
						player.getDialogueManager().startDialogue("SimpleNPCMessage", THHAAR_MEJ_JAL, "Some of Harmony's most notourious players will now be up against you. Goodluck.");
	...
	public void startWave() {
		...
		for(int id : WAVES[currentWave-1]) {
			if(id == 2736)
				...
				new PlayerWarsFinal(id, getSpawnTile(), this);
			else
	...
	public void setWaveEvent() {
		if(getCurrentWave() == 63) 
			player.getDialogueManager().startDialogue("SimpleNPCMessage", THHAAR_MEJ_JAL, "Look out, here comes Dryax!");
		CoresManager.fastExecutor.schedule(new TimerTask() {
	...
	public void exitCave(int type) {
		...
		if(type == 0 || type == 2)
			...
			if(type == 1 || type == 4) {
				...
				if(type == 4) {
					player.reset();
					player.getDialogueManager().startDialogue("SimpleNPCMessage", THHAAR_MEJ_JAL, "You even defeated Dryax, I am most impressed! Please accept this gift as a reward.");
					SeasonEvent.HandleActivity(player, "Player Wars", 150);
					player.getPackets().sendGameMessage("You were victorious!!");
					player.pwamountcompleted ++;
					World.sendWorldMessage("<col=993300>"+player.getDisplayName()+" has completed Player Wars "+player.pwamountcompleted+" times.", true);
					//if(!player.getInventory().addItem(6570, 1)) {//change to points
					player.playerwarspoints += Settings.EVENT_SPOTLIGHT.contains("minigame") ? 200 : 100;
					if (player.playerwarsperk == true) {
						player.playerwarspoints += Settings.EVENT_SPOTLIGHT.contains("minigame") ? 400 : 200;
						player.sendMessage("Your perk doubles your points!");
					}
				//	}else if(!player.getInventory().addItem(6529, 16000)) 
					//	World.addGroundItem(new Item(6529, 16000), new WorldTile(player), player, true, 180,true);
					...
					player.playerwarspoints += Settings.EVENT_SPOTLIGHT.contains("minigame") ? 50 : 25;
					player.getDialogueManager().startDialogue("SimpleNPCMessage", THHAAR_MEJ_JAL, "Well done. Heres a few points as a reward.");
					//TODO tokens
				}
			}
			removeControler();
