...
public class Wilderness extends Controler {
	...
	public boolean processItemTeleport(WorldTile toTile) {
		int wildlevel = player.getTemporaryAttributtes().get("GloryTeleport") == Boolean.TRUE ? 30 : 20;
		if (getWildLevel() > wildlevel) {
			player.getPackets().sendGameMessage(
					"A mysterious force prevents you from teleporting.");
			return false;
		}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					if (!player.getSession().getChannel().isReadable()) {
	...
	public void moved() {
		...
		//i//f(!BountyHunterManager.handlingPlayer(player))
	...
	public static final boolean isAtWild(WorldTile tile) {//TODO fix this
		boolean isAtWild = tile.getX() >= 3011 && tile.getX() <= 3132
				...
				|| World.HungerGames(tile) || World.DrygonicPvp1(tile) || World.Scorpia(tile) || World.DrygonicPvp2(tile) || World.DrygonicPvp3(tile);
        if (isAtWild && tile instanceof Bot && ((Bot) tile).getControlerManager().getControler() == null) {
			((Bot) tile).getControlerManager().startControler("Wilderness");
		}
		return isAtWild;
	}
