...
import com.rs.game.npc.others.Death;
import com.rs.game.npc.others.DeathNPC;
import com.rs.game.npc.others.Lucien;

public class Deathgame extends Controler {

	public static final WorldTile OUTSIDE = new WorldTile(3163,3478, 0);

	private static final int Lucien = 8443;
	...
	public static void enterFightCaves(Player player) {
		if(player.getFamiliar() != null || player.getPet() != null || Summoning.hasPouch(player) || Pets.hasPet(player)) {
			player.getDialogueManager().startDialogue("SimpleNPCMessage", Lucien, "No");
			return;
		}
		player.getControlerManager().startControler("Deathgame", 1); //start at wave 1
	}
	...
	public void loadCave(final boolean login) {
		...
		CoresManager.slowExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				boundChuncks = RegionBuilder.findEmptyChunkBound(1, 1); 
				//copys real map into the empty map
				//552 640
				RegionBuilder.copyAllPlanesMap(475, 376, boundChuncks[0], boundChuncks[1], 2);
				//RegionBuilder.copyAllPlanesMap(584, 644, boundChuncks[0], boundChuncks[1], 8);
				//RegionBuilder.copyAllPlanesMap(585, 643, boundChuncks[0], boundChuncks[1], 8);
				//selects a music
				selectedMusic = MUSICS[Utils.random(MUSICS.length)];
				player.setNextWorldTile(!login ? getWorldTile(8, 6) : getWorldTile(8, 6) );
				//1delay because player cant walk while teleing :p, + possible issues avoid
				WorldTasksManager.schedule(new WorldTask()  {
					@Override
					public void run() {
						if(!login) {
							WorldTile walkTo = getWorldTile(8, 6);
							player.addWalkSteps(walkTo.getX(), walkTo.getY());
						}
						player.getDialogueManager().startDialogue("SimpleNPCMessage", Lucien, "Enjoy the minigame!");
	...
	public WorldTile getSpawnTile() {
		...
		case 0:
			return getWorldTile(5, 9);
		case 1:
			return getWorldTile(10, 9);
		case 2:
			return getWorldTile(10, 3);
		case 3:
		default:
			return getWorldTile(5, 3);
		}
	}
	...
	public void startWave() {
		...
		for(int id : WAVES[currentWave-1]) {
			if(id == 1)
				new Death(id, getSpawnTile());
			else if (id == 14256)
				new Lucien(id, getSpawnTile());
			else
				new DeathNPC(id, getSpawnTile());
		}
		spawned = true;
	}
	...
	public void spawnHealers() {
		if(stage != Stages.RUNNING)
			return;
		for(int i = 0; i < 4; i++)
			new DeathNPC(2746, getSpawnTile());
	}
	...
	public void setWaveEvent() {
		if(getCurrentWave() == 63) 
			player.getDialogueManager().startDialogue("SimpleNPCMessage", Lucien, " Here I Come...");
		CoresManager.fastExecutor.schedule(new TimerTask() {
	...
	public void exitCave(int type) {
		...
		if(type == 0 || type == 2)
			...
			if(type == 1 || type == 4) {
				...
				if(type == 4) {
					player.isCompletedDeathgame();
					player.reset();
					player.getDialogueManager().startDialogue("SimpleNPCMessage", Lucien, "You even defeated Me, take this a Reward");
					...
					}else if(!player.getInventory().addItem(995, 16064 * 10)) 
						World.addGroundItem(new Item(995, 16064), new WorldTile(player), player, true, 180,true);
				}else if(getCurrentWave() == 1) 
					player.getDialogueManager().startDialogue("SimpleNPCMessage", Lucien, "Well I suppose you tried... better luck next time.");
				else{
					int tokkul = getCurrentWave() * 8032 / WAVES.length;
					tokkul *= 10; //10x more
					if(!player.getInventory().addItem(995, tokkul)) 
						World.addGroundItem(new Item(995, tokkul), new WorldTile(player), player, true, 180,true);
					player.getDialogueManager().startDialogue("SimpleNPCMessage", Lucien, "Well done in the cave, here, take Money as reward.");
					//TODO tokens
				}
			}
			removeControler();
