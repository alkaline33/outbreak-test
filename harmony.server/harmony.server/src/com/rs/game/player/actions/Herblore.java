...
public class Herblore extends Action {
	...
	public static final short OVERLOAD = 15333;
	public static final short PRENEW = 21632;
	public static final short SBREW = 6687;
	public static final short AORE = 29955;
	public static final short SHEAF = 29634;
	...
	public enum Ingredients {
		...
		AVANTOE(261, new short[] { Herblore.VIAL, 2436, 145, 147, 149 },
				new short[] { 103, 15308, 15309, 15310, 15311 }, new byte[] { 0, 88, 88, 88, 88 }, new double[] { 0, 220, 220, 220, 220 }),
		...
		LANTADYME(2481, new short[] { Herblore.VIAL, 2442, 163, 165, 167 },
				new short[] { 2483, 15316, 15317, 15318, 15319 }, new byte[] { 0, 90, 90, 90, 90 }, new double[] { 0, 240, 240, 240, 240 }),

		DWARF_WEED(267, new short[] { Herblore.VIAL, 2440, 157, 159, 161 },
				new short[] { 109, 15312, 15313, 15314, 15315 }, new byte[] { 0, 89, 89, 89, 89 }, new double[] { 0, 230, 230, 230, 230 }),

		CACTUS_SPINE(6016, new short[] { Herblore.COCONUT_MILK }, new short[] { 5936 }, new byte[] { 0 }, new double[] { 0 }),

		TORSTOL(269, new short[] { Herblore.VIAL, 15309, 15313, 15317, 15321, 15325 }, new short[] { 111, 15333, 15333, 15333, 15333, 15333 }, new byte[] { 0, 96, 96, 96, 96, 96 }, new double[] { 0, 1000, 1000, 1000, 1000, 1000 }),

		OVERPRAY(15332, new short[] { Herblore.PRENEW }, new short[] { 29770, 29770, 29770, 29770, 29770, 29770 }, new byte[] { 99, 99, 99, 99, 99, 99 }, new double[] { 800, 2000, 2000, 2000, 2000, 2000 }),

		SUPERBREW(Herblore.SBREW, new short[] { Herblore.AORE }, new short[] { 29766, 29766, 29766, 29766, 29766, 29766 }, new byte[] { 91, 91, 91, 91, 91, 91 }, new double[] { 800, 800, 800, 800, 800, 800 }),

		OVERLOADED(Herblore.OVERLOAD, new short[] { Herblore.SHEAF }, new short[] { 29633, 29633, 29633, 29633, 29633, 29633 }, new byte[] { 99, 99, 99, 99, 99, 99 }, new double[] { 1400, 1400, 1400, 1400, 1400, 1400 }),
		...
		UNICORN_HORN_DUST(235, new short[] { 93, 101 }, new short[] { 175, 181 }, new byte[] { 5, 48 }, new double[] { 37.5, 106.3 }),
	...
	public boolean start(Player player) {
		...
		if (ingredients != null) {
			slot = ingredients.getSlot(otherItem.getId());
			if (slot == -1) {
				slot = ingredients.getSlot(node.getId());
			}
	...
	public boolean process(Player player) {
		...
		if (otherItem.getId() == 12539 && node.getId() == 169) {
			if (!player.getInventory().containsOneItem(169) || !player.getInventory().containsItem(12539, 5)) {
				player.sendMessage(Colors.red + "You need at least 5 grenwall spikes per potion to make this!");
				return false;
			}
			return true;
		}
		if (otherItem.getId() == 9594 && node.getId() == 3042) {
			if (!player.getInventory().containsOneItem(3042) || !player.getInventory().containsItem(9594, 1)) {
				// player.sendMessage(Colors.red + "You need at least 5 grenwall spikes per
				// potion to make this!");
				return false;
			}
			return true;
		}
		if (otherItem.getId() == 12539 && node.getId() == 2444) {
			if (!player.getInventory().containsOneItem(2444) || !player.getInventory().containsItem(12539, 5)) {
				player.sendMessage(Colors.red + "You need at least 5 grenwall spikes per potion to make this!");
				return false;
			}
			return true;
		}
		if (otherItem.getId() == 9594 && node.getId() == 3040) {
			if (!player.getInventory().containsOneItem(3040) || !player.getInventory().containsItem(9594, 1)) {
				// player.sendMessage(Colors.red + "You need at least 5 grenwall spikes per
				// potion to make this!");
				return false;
			}
		...
		if (!player.getInventory().containsItem(node.getId(), node.getAmount()) || !player.getInventory().containsItem(otherItem.getId(), otherItem.getAmount())) {
			return false;
		}
		return true;
	}
	...
	public int processWithDelay(Player player) {
		...
		if (otherItem.getId() == VIAL || otherItem.getId() == COCONUT_MILK || node.getId() == VIAL || node.getId() == COCONUT_MILK) {
			player.getPackets().sendGameMessage("You add the " + node.getDefinitions().getName().toLowerCase().replace("clean", "") + " into the vial of " + (otherItem.getId() == VIAL ? "water." : "water."));
			...
			// World.sendWorldMessage("1", false);
			player.overloadsmade++;
			if (player.getInventory().removeItems(new Item(269), new Item(15309), new Item(15313), new Item(15317), new Item(15321), new Item(15325))) {
			}
		} else {
			player.getPackets().sendGameMessage("You mix the " + node.getDefinitions().getName().toLowerCase() + " into your potion.", true);
		}
		if (otherItem.getId() != 269 && node.getId() != 233) {
			player.getInventory().removeItems(new Item(node.getId(), 1), rawIngredient == null ? new Item(otherItem.getId(), otherItem.getId() == 12539 ? 5 : 1) : null);
		}
		if (node.getId() == 233) {
			player.getInventory().removeItems(new Item(otherItem.getId(), 1));
		}
		if (World.Level3Zone(player)) {
			player.getBank().addItem(rawIngredient != null ? rawIngredient.getCrushedItem() : new Item(ingredients.getRewards()[slot], 1), true);
		}
		player.getInventory().addItem(rawIngredient != null ? rawIngredient.getCrushedItem() : new Item(ingredients.getRewards()[slot], 1));
		if (Utils.random(1, 10) == 1 && player.getEquipment().getCapeId() == 29987) {
			player.getBank().addItem(rawIngredient != null ? rawIngredient.getCrushedItem() : new Item(ingredients.getRewards()[slot], 1), true);
			player.sendMessage("Your herblore master cape perk adds an extra potion to your bank.");
		} else if (Utils.random(1, 10) == 1 && player.getEquipment().getCapeId() == 20771 && player.getSkills().getXp(Skills.HERBLORE) >= 104273167) {
			player.getBank().addItem(rawIngredient != null ? rawIngredient.getCrushedItem() : new Item(ingredients.getRewards()[slot], 1), true);
			player.sendMessage("Your herblore master cape perk adds an extra potion to your bank.");
		}
		/*player.dailypotions++;
		if (player.dailypotions == 150) {
			player.sendMessage(Colors.lightGray + "<img=6>You have completed the Daily Money Maker: Make 150 potions! Your rewards have been placed in your bank.");
			player.getBank().addItem(207, 15, true);
			player.getBank().addItem(219, 15, true);
			player.getBank().addItem(2485, 15, true);
		...
		if (ticks > 0) {
			// World.sendWorldMessage("5", false);
			return 1;
		}
		return -1;
	}
