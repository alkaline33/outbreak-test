...
public class Smithing extends Action {
	...
	public enum ForgingBar {
		...
		ADAMANT(2361, 70, new Item[] { new Item(1211, 1), new Item(1357, 1),
				...
				new Item(1271, 1) }, new double[] { 135, 175, 220, 400 },
				new int[] { 66, 210, 267 }),
		...
		RUNE(2363, 85, new Item[] { new Item(1213, 1), new Item(1359, 1),
				...
				new Item(1275, 1) }, new double[] { 175, 250, 500, 375 },
				new int[] { 66, 210, 267 }),
		
		DRAGON(29550, 95, new Item[] { new Item(1215, 1), new Item(1434, 1),
				new Item(1149, 1), new Item(1305, 2), new Item(4587, 2),
				new Item(1187, 3), new Item(1377, 3), new Item(3140, 3),
				new Item(-1, 1), new Item(-1, 1), new Item(-1, 1),
				new Item(7158, 4), new Item(4087, 3), new Item(4585, 3),
				new Item(11732, 3),new Item(29548, 1),new Item(11230, 1),
				new Item(29593, 1),new Item(1215, 1),new Item(1215, 1),
				new Item(1215, 1),new Item(1215, 1),new Item(1215, 1),
				new Item(1215, 1),new Item(1215, 1),new Item(1215, 1),
				new Item(1215, 1),new Item(1215, 1),new Item(1215, 1),
				new Item(1215, 1),
				 }, new double[] { 270, 350, 750, 500 },
				new int[] { 66, 210, 267 }),
		
		DRUNE(29574, 85, new Item[] { new Item(1213, 1), new Item(1359, 1),
				new Item(1432, 1), new Item(1147, 1), new Item(9381, 1),
				new Item(1289, 1), new Item(824, 1), new Item(4824, 1),
				new Item(-1, 1), new Item(-1, 1), new Item(-1, 1),
				new Item(44, 1), new Item(1333, 1), new Item(9431, 1),
	...
	public int processWithDelay(Player player) {
		...
		if (player.challengeid == 37 && player.challengeamount > 0 && bar.getItems()[index].getId() == 1119) {
			DailyChallenges.UpdateChallenge(player);
		}
		if (player.challengeid == 38 && player.challengeamount > 0 && bar.getItems()[index].getId() == 1121) {
			DailyChallenges.UpdateChallenge(player);
		}
		if (player.challengeid == 39 && player.challengeamount > 0 && bar.getItems()[index].getId() == 1127) {
			DailyChallenges.UpdateChallenge(player);
		}
		if (bar.getItems()[index].getId() == 1127) 
			player.runebodysmithed ++;		
		if (Utils.random(1, 20) == 1 && player.getEquipment().getCapeId() == 29978) {
			player.sendMessage("Your smithing master cape perk has saved some materials.");
		} else if (Utils.random(1, 20) == 1 && player.getEquipment().getCapeId() == 20771 && player.getSkills().getXp(Skills.SMITHING) >= 104273167) {
			player.sendMessage("Your smithing master cape perk has saved some materials.");
		} else {
		...
		if (bar.getBarId() == 29574) {
			player.getSkills().addXp(Skills.SMITHING, getExperience(player) / 10 * 9);
		} else {
	...
	public boolean start(Player player) {
		...
		if (!player.getInventory().containsItemToolBelt(HAMMER, bar.getBarId())) {
			player.getDialogueManager().startDialogue(
					"SimpleMessage",
					"You need a hammer in order to work with a bar of "
							+ new Item(bar.getBarId(), 1).getDefinitions()
