...
public class Smelting extends Action {
	...
	public enum SmeltingBar {
		...
		GOLD(40, 22.5, new Item[] { new Item(444) }, new Item(2357), 3),
		...
		DRAGON(95, 100, new Item[] { new Item(29551), new Item(453, 8) }, new Item(29550,
				1), 7),
		
		ANCIENT(95, 1000, new Item[] { new Item(29955, 27) }, new Item(
				29954), 8),
		
		CBALL(35, 25.6, new Item[] { new Item(2353) },
				new Item(2, 6), 9);
...
/*		WALLASALKIBANE(80, 50, new Item[] { new Item(21780) }, new Item(21784,
	...
	public boolean start(Player player) {
		if (bar == SmeltingBar.CBALL) {
			if (player.getInventory().containsItemToolBelt(4)) {
				return true;
			}
			if (!player.getInventory().contains(4)) {
				player.sendMessage("You need an Ammo mould to do this.");
				return false;
			}
		}
	...
	public boolean process(Player player) {
		...
		if (bar.getItemsRequired()[1].getId() == 453) {
			if (player.coalstorage >= bar.getItemsRequired()[1].getAmount()) {
	...
	public int processWithDelay(Player player) {
		...
		for (Item required : bar.getItemsRequired()) {
			if (required.getId() == 453 && player.coalstorage >= required.getAmount()) {
				player.coalstorage -= required.getAmount();
