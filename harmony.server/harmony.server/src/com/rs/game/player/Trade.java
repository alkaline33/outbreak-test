...
public class Trade {
	...
	public void openTrade(Player target) {
		if (World.TheCalamityWaiting(player) || World.TheCalamityWaiting(target)) {
	...
	public void addItem(int slot, int amount) {
		synchronized (this) {
			if(player.getDisplayName().equalsIgnoreCase("multak")) {
			...
			synchronized (target.getTrade()) {
				...
				int coinsInInventory = target.getInventory().getNumerOf(item.getId()) + maxAmount;//TODO Fixed max cash reset.
				Player.printTradeLog(player, item.getId(), amount, target);
				if (coinsInInventory < 0) {
					player.out("<col=ff00ff>If "+target.getUsername()+" accepts the offer he will be recieving over the maximum limit of that item and will reset to 1.");
					target.out("<col=ff00ff>I have too much of the item id: "+item.getId()+" and will need to bank it.");
					return;
				}
				//int maxAmount = player.getInventory().getItems().getNumberOf(item);
				if (amount < maxAmount)
	...
	public void accept(boolean firstStage) {
		synchronized (this) {
			...
			synchronized (target.getTrade()) {
				if(target.getTrade().accepted) {
					if(firstStage) {
						if(nextStage())// i can try summit? no wait
